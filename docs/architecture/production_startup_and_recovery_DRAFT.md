...
Version: Draft 0.8 - Phase 3C1-R2 F6 targeted normative remediation

Status: **DRAFT - NOT CANONICAL - NOT APPROVED**

Governing Dependency: approved ADR-014 and canonical ADR-012. ADR-015, ADR-016, this startup contract, and coordinated Runtime Authority amendments remain unapproved drafts pending independent Phase 3C1-R2 review; canonical incorporation is unauthorized.
...
| `CONTROL_STORES_VERIFIED` | ADR-014 Entry Agent Session Commit Writer for the session store; Runtime Authority Store Integrity Classifier; Runtime Authority Recovery Evidence Writer for external evidence only | session store returns `VERIFIED_CURRENT`; runtime store read-only proof requires SQLite `>=3.43.1`, `application_id=0x52484C54`, `user_version=2`, `RANDLE_RUNTIME_AUTHORITY_SCHEMA_V2`, accepted canonical schema hash, `RASTORE-BOOTSTRAP-V2`, writer-registry version `2`/accepted hash, all required PRAGMAs, exactly 40 STRICT tables/552 columns (485 nonnull/67 nullable)/133 FK declarations/173 mappings/one SHA-256 preflight view/13 partial unique indexes/21 triggers/62 active exclusive writer routes, valid successor sequences, deterministic/innocuous `randle_sha256_hex_utf8` preflight, `integrity_check=ok`, zero `foreign_key_check` rows, contiguous transaction/idempotency cursor, every store bootstrap/restore/reinitialization recovery row bound to its exact external prepared-evidence sequence/hash, and a verified bounded `RANDLE-RECOVERY-JCS-1` chain head with no unresolved prepared/quarantine/recovery state | URI `mode=ro` + `query_only=ON`; register the governed hash function and require `SELECT empty_sha256 FROM randle_sha256_preflight_v` to return the standard empty-input digest, execute Runtime Authority Store Schema sections 2-3/6-10/13-14 and compare committed Phase 3C1-R2 hashes; assert transaction/metadata counts unchanged by validation | any `STORE-*` result in section 6.3, function/view/hash/count/action/trigger/writer/succession/evidence-reference mismatch, `RECOVERY_REQUIRED`, unapproved recovery/bootstrap, unidentified legacy source, invalid/capacity-exhausted external chain, ambiguous transaction/process, or shared source | Required before any authority-dependent start |
...
| 5 | `STORE-WRITER-REGISTRY-INVALID` | registry version/hash, any of 60 table/operation scopes, active exclusivity, strict predecessor-retirement/successor sequence, serialization-safe contract/build identity, or row differs from Runtime Authority Store Schema sections 7 and 14.2 |
...
Current semantic traceability is incomplete. `Architecture/Audits/2026-07-17_ADR015_016_Clause_Traceability_Registry_DRAFT.md` is historical rejected Phase 3B evidence, not a current forward/reverse mapping. Phase 3C2 will rebuild clause/scenario/assertion traceability only from independently accepted Phase 3C1-R2 hashes. The external recovery matrix remains a package-level index and is not a substitute.
