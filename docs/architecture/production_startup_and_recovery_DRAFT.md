...
Version: Draft 0.7 - Phase 3C1-R1 targeted normative remediation

Status: **DRAFT - NOT CANONICAL - NOT APPROVED**

Governing Dependency: approved ADR-014 and canonical ADR-012. ADR-015, ADR-016, this startup contract, and coordinated Runtime Authority amendments remain unapproved drafts pending independent Phase 3C1-R1 review; canonical incorporation is unauthorized.
...
| `CONTROL_STORES_VERIFIED` | ADR-014 Entry Agent Session Commit Writer for the session store; Runtime Authority Store Integrity Classifier; Runtime Authority Recovery Evidence Writer for external evidence only | session store returns `VERIFIED_CURRENT`; runtime store read-only proof requires SQLite `>=3.43.1`, `application_id=0x52484C54`, `user_version=2`, `RANDLE_RUNTIME_AUTHORITY_SCHEMA_V2`, accepted canonical schema hash, `RASTORE-BOOTSTRAP-V2`, writer-registry version `2`/accepted hash, all required PRAGMAs, exactly 38 STRICT tables/500 columns (440 nonnull/60 nullable)/124 FK declarations/132 mappings/13 partial unique indexes/14 triggers/60 active exclusive writer routes, valid successor sequences, `quick_check=ok`, zero `foreign_key_check` rows, contiguous transaction/idempotency cursor, every store bootstrap/restore/reinitialization recovery row bound to its exact external prepared-evidence sequence/hash, and a verified bounded `RANDLE-RECOVERY-JCS-1` chain head with no unresolved prepared/quarantine/recovery state | URI `mode=ro` + `query_only=ON`; execute Runtime Authority Store Schema sections 2-3/6-10/13-14 and compare committed Phase 3C1-R1 hashes; assert transaction/metadata counts unchanged by validation | any `STORE-*` result in section 6.3, hash/count/action/trigger/writer/succession/evidence-reference mismatch, `RECOVERY_REQUIRED`, unapproved recovery/bootstrap, unidentified legacy source, invalid/capacity-exhausted external chain, ambiguous transaction/process, or shared source | Required before any authority-dependent start |
