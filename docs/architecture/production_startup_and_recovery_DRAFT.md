...
Version: Draft 0.9 - Phase 3C1-R3 F6 targeted normative remediation

Status: **DRAFT - NOT CANONICAL - NOT APPROVED**

Governing Dependency: approved ADR-014 and canonical ADR-012. ADR-015, ADR-016, this startup contract, and coordinated Runtime Authority amendments remain unapproved drafts pending independent Phase 3C1-R3 review; canonical incorporation is unauthorized.
...
For the Pattern A Runtime Authority Control Database, integrity, writer routing, foreign keys, quarantine, restore, reinitialization, initial bootstrap, rollback, and external evidence SHALL follow ADR-015 section 3.3.1, ADR-016 sections 3.6.1-3.6.7, and Runtime Authority Store Schema sections 2 through 14. The startup evidence manifest SHALL include store UUID; resolved database/sidecars; `user_version=2`; schema identity/hash; bootstrap identity; writer-registry version/hash; exact table/column/FK/action/index/trigger counts; last transaction cursor; supervisor/listener/bridge ancestry; listener current/transition/incident/outcome/acknowledgement proofs; Bridge Generation Writer provenance; health/subscription proofs; recovery incident; external evidence-chain head; candidate/source hashes; authorization identities; and activation record as applicable.

No restoration, reinitialization, bootstrap, import, or migration is automatic. No approved predecessor schema exists; legacy/unidentified stores are quarantined and supply no positive authority. Shared/OneDrive projection JSON, Command Center/status output, logs, process memory, a copied snapshot, or operator-edited records SHALL NOT be a control restoration source. When no qualifying owner-produced or governance-controlled backup exists, startup SHALL record `CONTROL_STORE_RECOVERY_REQUIRED`, remain terminally `FAILED`, leave public exposure and listener start prohibited, and require a separately governed operator reinitialization plan.

An approved restoration SHALL preserve the source store UUID, durable cursor, incident/fence/execution history, and listener/bridge ancestry, append its exact restore transaction/evidence, then allocate a new supervisor generation without importing prior freshness. Bootstrap and reinitialization SHALL construct exactly the negative candidate row sets and insertion order in Store Schema section 14.7; they may not claim success with only metadata/registry/health rows while required listener/recovery/transaction parents are absent. Reinitialization creates explicit listener version 1 `SUPERVISOR_STORE_FAILED` and negative health/recovery/incident rows, so the first post-replacement mutation is `TX-SUP-GENERATION-CREATE` against metadata cursor 1 rather than a CAS against an absent generation/state. Entry Session reinitialization uses `TX-ENTRY-STORE-RECOVERY-INITIALIZE` to insert `NO_CURRENT_SESSION_CONTEXT` version 1; restore preserves the exact CAS source version. A future migration is prohibited until an exact predecessor-bound contract is separately approved and is not a current startup verification obligation. Any cursor gap, ambiguous action, prepared-without-completed external evidence record, invalid evidence chain, or unresolved quarantine makes the candidate ineligible.
...
| `CONTROL_STORES_VERIFIED` | ADR-014 Entry Agent Session Commit Writer for the session store; Runtime Authority Store Integrity Classifier; Runtime Authority Recovery Evidence Writer for external evidence only | session store returns `VERIFIED_CURRENT`; runtime store read-only proof requires SQLite `>=3.43.1`, `application_id=0x52484C54`, `user_version=2`, `RANDLE_RUNTIME_AUTHORITY_SCHEMA_V2`, accepted canonical schema hash, `RASTORE-BOOTSTRAP-V2`, writer-registry version `2`/accepted hash, all required PRAGMAs, exactly 47 STRICT tables/670 columns (591 nonnull/79 nullable)/152 FK declarations/203 mappings/one SHA-256 preflight view/14 partial unique indexes/one deterministic expression index/27 triggers/70 active exclusive writer routes, valid successor sequences, deterministic/innocuous `randle_sha256_hex_utf8` preflight, `integrity_check=ok`, zero `foreign_key_check` rows, contiguous transaction/idempotency cursor, every store bootstrap/restore/reinitialization recovery row bound to its exact external prepared-evidence sequence/hash, and a verified bounded `RANDLE-RECOVERY-JCS-1` chain head with no unresolved prepared/quarantine/recovery state | URI `mode=ro` + `query_only=ON`; register the governed hash function before any schema-owned object access; require the direct empty-input output, `SELECT empty_sha256 FROM randle_sha256_preflight_v`, and presence/definition of `ix_randle_sha256_deterministic_guard`; execute Runtime Authority Store Schema sections 2-3/6-10/13-14 and compare committed Phase 3C1-R3 hashes; assert transaction/metadata counts unchanged by validation | any `STORE-*` result in section 6.3, missing/wrong/non-innocuous/nondeterministic function registration, view/hash/count/action/trigger/writer/succession/evidence-reference mismatch, `RECOVERY_REQUIRED`, unapproved recovery/bootstrap, unidentified legacy source, invalid/capacity-exhausted external chain, ambiguous transaction/process, or shared source | Required before any authority-dependent start |
...
| `SUPERVISOR_AUTHORITY_READY` | Listener Supervisor State Evaluator; Supervisor Generation, Listener Epoch, Listener State, Listener Incident, Listener Acknowledgement, Recovery Transaction, Bridge Generation, and Health Durable Writers | exactly one current supervisor generation/held lease created by the typed generation/lease operations; one exact `listener_current` and matching transition cursor/version; exact current epoch or state-permitted NULL; exact current/nonterminal incident or terminal own-outcome/same completion-and-recovery-transaction relationship; execution/rehydration/recovery ancestry; for `REHYDRATING`, the five required domains and exactly one same-generation/epoch positive acknowledgement each; one current `bridge_generations` row carrying `BRIDGE_GENERATION_WRITER`, matching bridge current/transition/incident/outcome; five exact health rows/aggregate; positive NQ/YM subscription rows only in `subscription_verifications`; and, when a termination observation exists for the current listener process, the latest observation has exactly one schema-v2 result/set, six producer windows whose rows all match durable cursor acceptance, exactly five direct roles, current generation/epoch/bridge/process/observation, cutoff-to-record delay at most 30 seconds, valid normalized-payload/evidence/set/result hashes, exact role-bound optional identities, no unresolved supporting uncertainty or conflict, and no field `UNKNOWN`; no open store incident or unresolved external recovery/quarantine state | read-only joins over executable SQL keys/FKs/indexes/triggers; recompute every normalized payload, evidence, set, and result hash; verify each producer window against its durable cursor and ingest transaction/idempotency chain; verify every supporting row including `UNKNOWN`; verify exact optional-role/domain mapping and normalized concrete/absence predicates; verify last transaction cursor/writer IDs and `TX-LSN-CANCEL`/terminal-incident predicates; rerun zero-row `foreign_key_check`; startup may neither repair nor reinterpret termination classification and Command Center supplies no acknowledgement | any `SUPERVISOR-*`, `LISTENER-*`, `ACK-*`, `BRIDGE-*`, `HEALTH-*`, `SUBSCRIPTION-*`, or `TERMINATION-*` currentity/completeness/conflict/integrity failure, supporting uncertainty, self-declared label without payload, wrong-role optional identity, non-cursor evidence, stale generation/epoch/bridge/process/observation, wrong writer, duplicate current, cursor, parent, version, terminal-outcome, acknowledgement, or recovery ambiguity | Required |
| `SHARED_FEED_POLICY_VALID` | ADR-015 Listener Supervision Policy Evaluator; Architecture Governance; Deployment Authorization Owner | durable `POLICY_VALID` result binding exact canonical artifact bytes/hash, schema/policy/topology identity, allowed values/ranges, evaluator build, approval, deployment authorization, and startup attempt | compare the policy-validation result to the deployment artifact and current `shared_feed_policies` row committed by `TX-POLICY-VALIDATE` in the Runtime Authority Store | `SHARED_FEED_POLICY_INVALID`, missing/stale result, digest/version/range/approval mismatch, or attempted override | Required before listener/bridge grant; invalid policy terminates startup without restart |
...
| `BRIDGE_GENERATION_GRANTED` | Listener Supervisor State Evaluator as sole grant authority; Bridge Generation Writer as sole `bridge_generations` writer; Bridge Controller as evidence producer only | one current Bridge Generation within the granted listener epoch, row `writer_id='BRIDGE_GENERATION_WRITER'`, active registry ownership for `bridge_generations` INSERT/UPDATE, and authenticated Controller acknowledgement of the exact grant/capability | compare durable generation/grant transaction and registry route to authenticated Controller evidence and exact current identities | wrong writer/registry route, generation mismatch, old capability, unfenced duplicate, missing Supervisor grant, or acknowledgement represented as grant/write authority | Required |
...
| `SYMBOLS_SUBSCRIBED` | Rithmic listener authenticated `SUBSCRIPTION_PROOF_OBSERVED` producer; Health Ingress; Listener Supervisor State Evaluator; Health Durable Writer | one positive `subscription_verifications` row for each resolved current NQ/YM contract after current bridge grant; no `SUBSCRIPTION_VERIFIED` token in `health_events`; any health-state consequence has a separate `TX-HEALTH-DIMENSION-UPDATE` | authenticate/sequence `SUBSCRIPTION_PROOF`, verify exact contract/session/supervisor/epoch/bridge/producer/request/freshness, inspect evaluator decision, and read back `subscription_verifications` plus any separate health transaction | missing/wrong contract, producer self-assertion, result stored in `health_events`/`health_current` instead of its own row, one symbol unverified, old generation, request return/`ACTIVE` without committed verification | Required |
...
### 6.3 Runtime Authority Store startup failure results

The store validator SHALL return the first failure in the fixed order below and SHALL retain every additional finding in the evidence manifest. Every result terminates startup `FAILED`; none permits partial readiness or process-existence fallback.

| Order | Result | Exact failed proof |
|---:|---|---|
| 1 | `STORE-SCHEMA-IDENTITY-MISMATCH` | SQLite `<3.43.1`, application ID, `user_version=2`, schema identity/hash, bootstrap identity, object count, or normalized executable SQL schema differs |
| 2 | `STORE-FOREIGN-KEYS-DISABLED` | coordinator connection does not report `PRAGMA foreign_keys=1` |
| 3 | `STORE-FOREIGN-KEY-CHECK-FAILED` | `PRAGMA foreign_key_check` returns any row |
| 4 | `STORE-INTEGRITY-CHECK-FAILED` | `PRAGMA quick_check` does not return sole value `ok` |
| 5 | `STORE-WRITER-REGISTRY-INVALID` | registry version/hash, any of 70 table/operation scopes, active exclusivity, strict predecessor-retirement/successor sequence, serialization-safe contract/build identity, or row differs from Runtime Authority Store Schema sections 7 and 14.2 |
| 6 | `STORE-TRANSACTION-CURSOR-AMBIGUOUS` | `transaction_commits` is noncontiguous, metadata cursor differs, idempotency record conflicts, or WAL recovery cannot classify commit/no-commit |
| 7 | `STORE-OPEN-RECOVERY-INCIDENT` | corruption/schema/routing/recovery `store_incidents` remains OPEN, the external evidence chain is invalid/capacity-exhausted, or a prepared bootstrap/quarantine/restore/reinitialize record lacks a verified terminal record |
| 8 | `SUPERVISOR-GENERATION-AMBIGUOUS` | zero or multiple CURRENT supervisor generations, zero/multiple HELD leases, or generation/lease parent mismatch |
| 9 | `LISTENER-CURRENT-AMBIGUOUS` | missing/duplicate `listener_current`, invalid state/version, or wrong writer/update transaction |
| 10 | `LISTENER-EPOCH-ANCESTRY-INVALID` | applicable listener epoch missing, duplicated, stale, fenced, or not parented to the current generation |
| 11 | `LISTENER-TRANSITION-CURSOR-INVALID` | `listener_current.last_transition_id`, prior/result states, expected/result versions, or transaction identity do not agree |
| 12 | `LISTENER-INCIDENT-RELATIONSHIP-INVALID` | current incident/outcome/fence/execution/rehydration/recovery relationship is missing, duplicated, terminally inconsistent, or wrong generation/epoch |
| 13 | `ACKNOWLEDGEMENT-SET-INCOMPLETE` | `REHYDRATING` required-domain set is missing, Command Center appears, an accepted identity differs, or any required domain lacks exactly one accepted acknowledgement |
| 14 | `BRIDGE-GENERATION-AMBIGUOUS` | current Bridge Generation/current state/incident/transition is missing, duplicated, stale, wrong listener epoch, or `bridge_generations` is not written through the active Bridge Generation Writer route |
| 15 | `HEALTH-DIMENSION-INVALID` | any exact `health_current` primary-key row is missing or invalid: (`PERSISTENCE`,`GLOBAL`), (`TRANSPORT`,`NQ_YM_SHARED_FEED`), (`AUTHENTICATION`,`NQ_YM_SHARED_FEED`), (`AUTHORITY_COHERENCE`,`NQ_YM_SHARED_FEED`), (`TIME_AUTHORITY`,`GLOBAL`); duplicate, wrong-domain/scope, stale-generation, or transition/version inconsistency also fails; subscription proof remains the separate `SYMBOLS_SUBSCRIBED` gate |
| 16 | `HEALTH-AGGREGATE-MISMATCH` | recomputation from all five health dimensions does not equal `health_aggregate` state/hash/version |
| 17 | `STALE-GENERATION-POSITIVE-EVIDENCE` | subscription, termination, expectation, bridge, health, or acknowledgement positive fact names a noncurrent generation/epoch/generation |
| 18 | `DUPLICATE-CURRENT-ROW` | any partial-unique singleton/current invariant is violated even if a preferred row could be selected |
| 19 | `CONTROL-STORE-RECOVERY-REQUIRED` | a qualifying source, approval, v2 bootstrap, restore, reinitialization, quarantine, activation, or separately governed future migration remains unresolved; legacy/unidentified data cannot satisfy it |

`READY_LOCKED` SHALL NOT be evaluated until both store gates have passed every row above and their positive validation identities are present in the startup evidence manifest.
...
- health-store corruption detection, verified quarantine, quarantine failure, qualifying/nonqualifying restore, no-source recovery-required, clean bootstrap, reinitialization, pre/post-activation rollback boundary, first post-replacement initialization/CAS, epoch/generation preservation, and audit failure;
...
Current semantic traceability is incomplete. `Architecture/Audits/2026-07-17_ADR015_016_Clause_Traceability_Registry_DRAFT.md` is historical rejected Phase 3B evidence, not a current forward/reverse mapping. Phase 3C2 will rebuild clause/scenario/assertion traceability only from independently accepted Phase 3C1-R3 hashes. The external recovery matrix remains a package-level index and is not a substitute.
