...
Version: Draft 0.6 - Phase 3C1 normative remediation

Status: **DRAFT - NOT CANONICAL - NOT APPROVED**

Governing Dependency: approved ADR-014 and canonical ADR-012. ADR-015, ADR-016, this startup contract, and coordinated Runtime Authority amendments remain unapproved drafts pending independent Phase 3C1 review; canonical incorporation is unauthorized.
...
An approved restoration SHALL preserve the source store UUID, durable cursor, incident/fence/execution history, and listener/bridge ancestry, then allocate a new supervisor generation without importing prior freshness. A reinitialization SHALL use a new store UUID, import no positive authority, reconcile exact runtime/OS state, and remain blocked until explicitly activated. A future migration is prohibited until an exact predecessor-bound contract is separately approved. Any cursor gap, ambiguous action, prepared-without-completed external evidence record, invalid evidence chain, or unresolved quarantine makes the candidate ineligible.
...
| `CONTROL_STORES_VERIFIED` | ADR-014 Entry Agent Session Commit Writer for the session store; Runtime Authority Store Integrity Classifier; Runtime Authority Recovery Evidence Writer for external evidence only | session store returns `VERIFIED_CURRENT`; runtime store read-only proof requires SQLite `>=3.43.1`, `application_id=0x52484C54`, `user_version=2`, `RANDLE_RUNTIME_AUTHORITY_SCHEMA_V2`, accepted canonical schema hash, `RASTORE-BOOTSTRAP-V2`, writer-registry version `2`/accepted hash, all required PRAGMAs, exactly 37 STRICT tables/468 columns/115 FK declarations/117 mappings/13 partial unique indexes/11 triggers/59 active exclusive writer routes, `quick_check=ok`, zero `foreign_key_check` rows, contiguous transaction/idempotency cursor, and verified external evidence-chain head with no unresolved prepared/quarantine/recovery state | URI `mode=ro` + `query_only=ON`; execute Runtime Authority Store Schema sections 2-3/6-10/13 and compare committed Phase 3C1 hashes; assert transaction/metadata counts unchanged by validation | any `STORE-*` result in section 6.3, hash/count/action/trigger/writer mismatch, `RECOVERY_REQUIRED`, unapproved recovery/bootstrap, unidentified legacy source, invalid external chain, ambiguous transaction/process, or shared source | Required before any authority-dependent start |
...
| `SUPERVISOR_AUTHORITY_READY` | Listener Supervisor State Evaluator; Supervisor Generation, Listener Epoch, Listener State, Listener Incident, Listener Acknowledgement, Recovery Transaction, Bridge Generation, and Health Durable Writers | exactly one current supervisor generation/held lease; one exact `listener_current` and matching `listener_state_transitions` cursor/version; exact current epoch or state-permitted NULL; exact current/nonterminal incident or terminal `listener_restart_outcomes` relationship; execution/rehydration/recovery ancestry; for `REHYDRATING`, the five required domains and exactly one same-generation/epoch positive acknowledgement each; one current `bridge_generations` row carrying `BRIDGE_GENERATION_WRITER`, matching bridge current/transition/incident/outcome; five exact health rows/aggregate; positive NQ/YM subscription rows only in `subscription_verifications` with authenticated `SUBSCRIPTION_PROOF`; no open store incident or unresolved external recovery/quarantine state | read-only joins over the executable SQL keys/FKs/indexes/triggers, verify last transaction cursor/writer IDs/evidence hashes, and rerun zero-row `foreign_key_check`; Command Center supplies no acknowledgement | any `SUPERVISOR-*`, `LISTENER-*`, `ACK-*`, `BRIDGE-*`, `HEALTH-*`, `SUBSCRIPTION-*`, stale-generation, wrong writer, duplicate-current, cursor, parent, version, terminal-outcome, acknowledgement, or recovery ambiguity result | Required |
...
| 5 | `STORE-WRITER-REGISTRY-INVALID` | registry version/hash, any of 59 table/operation scopes, active exclusivity, effective/retired sequence, writer contract/build identity, or row differs from Runtime Authority Store Schema section 7 |
...
| 7 | `STORE-OPEN-RECOVERY-INCIDENT` | corruption/schema/routing/recovery `store_incidents` remains OPEN, the external evidence chain is invalid, or a prepared bootstrap/quarantine/restore/reinitialize/future-migration record lacks a verified terminal record |
...
Current semantic traceability is incomplete. `Architecture/Audits/2026-07-17_ADR015_016_Clause_Traceability_Registry_DRAFT.md` is historical rejected Phase 3B evidence, not a current forward/reverse mapping. Phase 3C2 will rebuild clause/scenario/assertion traceability only from independently accepted Phase 3C1 hashes. The external recovery matrix remains a package-level index and is not a substitute.
