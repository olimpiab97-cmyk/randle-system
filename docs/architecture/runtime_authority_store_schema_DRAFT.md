...
Version: Draft 0.6 — Phase 3C1-R3 F6 targeted normative remediation
...
Every one of the 47 tables is `STRICT`. Every declared type is one of `INTEGER`, `REAL`, `TEXT`, `BLOB`, or `ANY`; this schema currently needs only `INTEGER` and `TEXT`. There are no declared aliases such as `UUID`, `SHA256`, `UTC`, `SEQ`, or `VERSION`.
...
Phase 3C1-R3 published value: `304dce825d2542b8c7a69f56b0322376daa8fe2670848e0bbfbef528b12a4180`.
...
The block includes, in file order, all 47 `CREATE TABLE` definitions, the one schema-owned `randle_sha256_preflight_v` view, inline primary/unique/check/foreign-key constraints, all 14 named partial unique indexes, the deterministic-registration expression index, and all 27 triggers. It excludes PRAGMAs, the preliminary temporary-table UDF call, the surrounding DDL transaction, writer-registry data rows, and comments outside the block.
...
Phase 3C1-R3 published value: `7ab99f5802bb6d9e20f73b2ecbf9372ce5cf8fbd093e0d9cf1bd571bf1c31c28`.
...
The committed SQL-artifact byte hash is a separate provenance identity and is not substituted for either canonical hash. Phase 3C1-R3 committed SQL SHA-256: `bed772dfcfaffed0a5ad19176e560cebd139f19fce555482cbce6f7320e43636`.
...
The executable contract has 47 tables and 670 columns: 591 are `NOT NULL` and 79 are nullable. It has 47 primary keys covering 56 primary-key columns. R3 adds the durable `termination_producer_cursors` and six normalized domain-payload tables while preserving the R2 evidence-set/result tables. The recovery-binding columns from R1 remain mandatory together only for bootstrap/restore/reinitialization and prohibited for listener/bridge/cold-start recoveries. The primary key, exact nullability, all column checks, uniques, foreign keys, actions, and deferrability are expressed in the SQL, not inferred from this summary.
...
| `TX-TERMINATION-EVIDENCE-INGEST` | Health Ingress; Health Durable Writer | one authenticated normalized domain payload and evidence row plus exact producer cursor prior+1 update; `TERMINATION_EVIDENCE_RECORDED` |
...
1. exact schema v2, schema hash, registry version 2/hash, PRAGMAs, governed SHA-256 function preflight, 47 tables, 670 columns, 152 FK declarations/203 mappings, 14 partial unique indexes, one deterministic-registration expression index, and 27 triggers;
...
The operation names below are exhaustive for all 70 active routes. `COMMON` means every one of the 52 commit operations inserts its transaction/idempotency rows and updates the metadata cursor; it does not authorize a domain row. Candidate construction uses the named bootstrap/restore/reinitialize operation only.
...
| `termination_producer_cursors` | INSERT -> `TX-PRODUCER-REGISTER`; UPDATE -> `TX-TERMINATION-EVIDENCE-INGEST`, `TX-PRODUCER-RETIRE` |
...
| `termination_command_payloads`, `termination_os_process_payloads`, `termination_provider_payloads`, `termination_bridge_payloads`, `termination_listener_payloads`, `termination_derivation_payloads` | INSERT -> `TX-TERMINATION-EVIDENCE-INGEST` |
| `termination_evidence` | INSERT -> `TX-TERMINATION-EVIDENCE-INGEST` |
...
Each set contains exactly six producer-window rows, one each for `RITHMIC_LISTENER`, `BRIDGE_CONTROLLER`, `OS_ADAPTER`, `RAPI_ADAPTER`, `SUPERVISOR_ADAPTER`, and `OPERATOR_ADAPTER`. Every row in those windows must have been durably accepted through the cursor transaction in section 14.5.1; final numeric continuity is necessary but never substitutes for arrival-order proof.
...
Every optional result identity is either governed NULL or the exact role/domain contributor selected by the priority mapping in section 14.5.1. The result process and observation identities are mandatory and are composite-FK equal to all contributors. Cross-result, cross-set, stale generation/epoch/bridge, cross-process, cross-observation, wrong role/domain, or outside-the-five identity fails. Symbol/contract identities are intentionally inapplicable to this shared bridge/process termination classification.

`NONE` is field-specific content-derived absence, not a relationship or evidence label. The exact six-window recomputation and role-specific scopes are section 14.5.1; `PROCESS_EXCEPTION` can never be an absence proof.
...
Minimum concrete proof is closed by the normalized producer/role/evidence-type/payload predicates in section 14.5.1 and the executable trigger. A vocabulary token without the required authenticated payload fails; nonzero exit alone supplies no classified cause.

The Health State Evaluator owns conflict and uncertainty detection and the decision; the Health Durable Writer records only its authorized decision. The database validates but never originates the classification. Reclassification uses a new observation identity and next gapless sequence. A committed field-specific `UNKNOWN` blocks the termination-related startup proof until a later current nonconflicting classification satisfies it.

The R2 set/result hash profiles are superseded by the R3 profiles in section 14.5.1. No profile-2 termination set or result is current-schema input.

#### 14.5.1 Phase 3C1-R3 termination-ingress and semantic closure

This subsection supersedes inconsistent R2 wording in section 14.5. `termination_producer_cursors` is the only durable arrival-order cursor. `TX-TERMINATION-EVIDENCE-INGEST` replaces the broader former evidence-operation name without increasing the closed 55-operation/52-commit count. It inserts exactly one normalized payload and its `termination_evidence` row, and atomically advances exactly one active producer cursor. The transaction row must carry request hash equal to the evidence hash, evidence-set hash equal to the normalized-payload hash, exact single writer set `HEALTH_DURABLE_WRITER`, and canonical expected/result JSON naming producer instance, cursor state version, producer sequence, and ingress sequence before/after. Their hashes must recompute, and the matching committed idempotency record must exist. The first row is `(1,1,version 1)`; every later row is prior+1 for all three counters. Same-key/same-hash retry returns the prior result without executing this insert; different bytes are `IDEMPOTENCY_CONFLICT`. A gap, duplicate, late/fenced instance, stale version, or concurrent loser rolls back. `TX-PRODUCER-RETIRE` fences the cursor; a replacement is a new registered instance starting at 1.

The normalized payload tables are closed and insert-only: `termination_command_payloads` binds authenticated principal, command/action/authorization, target identities, issue/accept times, generation and epoch; `termination_os_process_payloads` binds PID/start/process/exit/crash/method/command/handle and current identity tuple; `termination_provider_payloads` binds callback/type/RpCode/result/authentication/engine/connection and current tuple; `termination_bridge_payloads` binds current bridge state/recovery/action/completion; `termination_listener_payloads` binds current listener state/incident/requested transition/outcome; `termination_derivation_payloads` binds role, producer, cutoff, exact window/cursor, and recomputable positive/uncertainty/conflict/missing counts. `trg_termination_evidence_payload_semantics` joins the payload to its governed producer role and exact evidence type and maps the asserted vocabulary to payload content. A label without the required payload fails. Nonzero exit alone cannot satisfy `BRIDGE_CRASH`; crash requires current matched crash/exception identity or the closed authoritative OS predicate.

Every supporting authenticated in-window assertion is semantically active. R3 intentionally defines no within-observation precedence override. If any row for a role asserts `UNKNOWN`, uses `assertion_kind=UNCERTAINTY`, or carries `INDETERMINATE`/`CONFLICT`, then that field must be `UNKNOWN` or insertion fails. Uncertainty propagation is role-local: Initiator -> Initiator, Requested Action -> Requested Action, Execution Method -> Execution Method, Observed Cause -> Observed Cause, Result -> Result. Planned-shutdown cross-field dependencies may additionally reject an otherwise field-local result. An unrelated field retains its independently proven token, but any field `UNKNOWN` blocks termination-related startup evidence. Resolution requires the next observation identity and sequence with an entirely current set.

Optional identity roles are exact and deterministic. `request_identity` is mandatory only for a concrete Requested Action and must be the direct `REQUESTED_ACTION_EVIDENCE` command payload. `operator_command_identity` is mandatory only for `AUTHENTICATED_OPERATOR` and must be the direct `INITIATOR_EVIDENCE` operator command. `process_exit_evidence_identity` is mandatory only for `PROCESS_EXITED` and must be direct OS-backed `RESULT_EVIDENCE`. Provider identity priority is Result `ENGINE_INERT`, provider-backed Cause, provider-backed Method, then `RAPI_PROVIDER` Initiator; OS identity priority is process-exit Result, OS-backed Cause, then OS-backed Method; bridge identity exists exactly when Result contribution is bridge-backed; listener identity priority is listener-backed Result, Cause, then Initiator. Each priority selects the first applicable direct contributor; otherwise the column is NULL. Cross-role, supporting-only, cross-set/result/producer/generation/epoch/bridge/process/observation, wrong-domain, or unauthenticated identity fails.

Content-derived `NONE` requires all six role-specific derivation payloads over the exact cursor-complete windows and an independent SQL scan proving zero positive, uncertainty, conflict, missing, rejected, stale, or mismatched evidence for that field. The stored disposition/counts are not self-proving. Concrete tokens require the exact normalized positive predicate enumerated in ADR-016 and the SQL trigger. Provider forced/logout/shutdown, planned command, supervisor terminate/kill, authenticated operator, full listener restart, process exit, and engine inert each require their specific current payload and direct role.

Canonical evidence JSON embeds the normalized payload as a JSON object and its hash. The set profile is `RANDLE-TERMINATION-EVIDENCE-SET-3`: header; six producer lines including last cursor version; every in-window evidence line including both sequences, cursor version, payload kind/ID/hash, and evidence hash; then five contributor lines. The result profile is `RANDLE-TERMINATION-RESULT-3`: all result identities including process-exit identity, classifications, times/version/transaction, followed by five role-ordered contributor payload identities/hashes. Lines use TAB separators and LF termination exactly as the triggers construct them; NULL optionals are `-`. Payload tampering therefore breaks payload, evidence, set, and result verification.

Every connection that creates or validates schema objects, evaluates triggers or the expression index, or performs authority reads/writes registers `randle_sha256_hex_utf8(TEXT)` with `SQLITE_UTF8|SQLITE_DETERMINISTIC|SQLITE_INNOCUOUS` before using the schema. The direct empty-input table check rejects missing/wrong output; the schema-owned view under `trusted_schema=OFF` rejects non-innocuous registration; `ix_randle_sha256_deterministic_guard` cannot be created without `SQLITE_DETERMINISTIC`. Only all three governed flags plus exact output pass.
...
For R3 termination rows, the dependency order within restore steps 5 and 6 is exact: insert producer registrations (creating zero cursors), insert the referenced listener epochs and bridge generations, then replay each authenticated normalized payload/evidence transaction in original producer-arrival order so the triggers reconstruct each cursor. This ordering supersedes any earlier wording that placed evidence replay before its epoch/bridge parents.
...
No current staged migration verification exists. Any migration transform, evidence type, or test is future-only under `FUTURE SEPARATELY GOVERNED PREDECESSOR-BOUND MIGRATION SPECIFICATION` and is not a Phase 3C1-R3 executable obligation.

## 15. Verification and governance boundary

Phase 3C1-R3 isolated validation must execute the SQL only against temporary databases; introspect every table/column/key/FK/action/index/trigger; run `integrity_check` and `foreign_key_check`; exercise supporting uncertainty, exact optional-role membership, normalized payload predicates, cursor arrival order/idempotency/restart/concurrency, all UDF flag variants, and prior F6 missing/duplicate/cross-identity/stale/gap/conflict/NONE/UNKNOWN/concrete/integrity cases; and rerun the accepted F1–F5/F7/F8 schema regressions without reopening their contracts.

Those are document/schema validation exercises only. They are not production implementation, runtime verification, deployment, readiness, or trading authorization. Full semantic clause-to-scenario traceability is intentionally deferred to Phase 3C2 and is not approval-ready. Phase 3C2 may use only hashes accepted by an independent Phase 3C1-R3 review.
