...
def log_supported_executor_actions():
    log(f"SUPPORTED_EXECUTOR_ACTIONS {list(SUPPORTED_EXECUTOR_ACTIONS)}")
...
def expire_stale_day_manual_exit_orders(reference_date=None):
    ...
    for order in ORDERS.values():
        if order.get("type") != "limit":
            continue
        if order.get("oco_role") != "manual_exit_limit":
            continue
        if str(order.get("tif") or "").upper() != "DAY":
            continue
        if str(order.get("status") or "").lower() not in WORKING_ORDER_STATUSES:
            continue
        session_date = order.get("session_date") or str(order.get("created_at") or "")[:10]
        if session_date and session_date < reference_date:
            order["status"] = "cancelled"
            order["cancelled_at"] = timestamp
            order["closed_reason"] = "day_manual_exit_expired"
            expired.append(order.get("order_id"))

    if expired:
        log(f"DAY MANUAL EXIT ORDERS EXPIRED orders={expired}")
    return expired
...
def load_executor_state():
    ...
    try:
        ...
        expired_orders = expire_stale_day_manual_exit_orders()
...
def active_trade_ids_for_symbol(symbol):
    return sorted({
        order.get("trade_id")
...
def ambiguous_symbol_scope_response(symbol, trade_ids):
    log(
        "ambiguous_trade_scope "
        f"symbol={symbol} active_trade_ids={trade_ids}"
    )
    return jsonify({
        "ok": False,
        "error": "ambiguous_trade_scope",
        "message": "ambiguous_trade_scope",
        "symbol": symbol,
        "active_trade_ids": trade_ids,
    }), 409
...
def find_position_symbol(symbol):
    raw_symbol = str(symbol or "").strip().upper()
    if raw_symbol in POSITIONS:
        return raw_symbol
    try:
        resolved_symbol, _ = resolve_execution_symbol(raw_symbol)
    except Exception:
    ...
    for candidate in POSITIONS:
        if normalize_symbol_root(candidate) == raw_root:
            return candidate
    return resolved_symbol or raw_symbol
...
def clear_working_orders_for_flat_symbol(symbol, reason, trade_id=None, oco_group=None):
    symbol = str(symbol or "").upper()
    if not symbol:
        return []
    ...
    if position_qty != 0:
        return []
    if not trade_id and not oco_group:
        trade_ids = active_trade_ids_for_symbol(symbol)
        if len(trade_ids) > 1:
            log(
                "ambiguous_trade_scope "
                f"action=clear_working_orders_for_flat_symbol symbol={symbol} "
                f"active_trade_ids={trade_ids}"
            )
            return []
    ...
    affected = []
    for order in active_orders_for_symbol(symbol):
        if trade_id and order.get("trade_id") != trade_id:
            continue
        if oco_group and order_oco_group(order) != oco_group:
            continue
        order["status"] = "cancelled"
        order["cancelled_at"] = timestamp
        order["closed_reason"] = reason
...
def evaluate_stop_fills_for_symbol(symbol, price, trade_id=None, order_id=None, oco_group=None):
    symbol = str(symbol or "").upper()
    if not symbol:
        return []
    ...
    for order in list(active_orders_for_symbol(symbol)):
        ...
        triggered_trade_id = order.get("trade_id")
        filled_qty = min(abs(float(order.get("qty", 0) or 0)), abs(position_qty))
        if position_qty < 0:
        ...
        if new_qty == 0:
            ...
            orphan_orders = clear_working_orders_for_flat_symbol(
                symbol,
                "cleared_after_stop_flat",
                trade_id=triggered_trade_id,
                oco_group=order_oco_group(order),
            )
...
def evaluate_limit_fills_for_symbol(symbol, price, trade_id=None, order_id=None, oco_group=None):
    symbol = str(symbol or "").upper()
    if not symbol:
        return []
    ...
    for order in list(active_orders_for_symbol(symbol)):
        ...
        if new_qty == 0:
            ...
            clear_working_orders_for_flat_symbol(
                symbol,
                "cleared_after_limit_flat",
                trade_id=triggered_trade_id,
                oco_group=order_oco_group(order),
            )
        else:
        ...
        trade_stop_qty = None
        active_trade_stops = [
            active_stop for active_stop in active_orders_for_trade(triggered_trade_id, "stop")
            if str(active_stop.get("symbol", "")).upper() == symbol
        ]
        if active_trade_stops:
            trade_stop_qty = float(active_trade_stops[0].get("qty", 0) or 0)
        remaining_qty = max((trade_stop_qty if trade_stop_qty is not None else abs(position_qty)) - filled_qty, 0.0)
...
def execute():
    ...
    if action == "submit_stop":
        ...
        if symbol in LAST_PRICES:
            stop_fills = evaluate_stop_fills_for_symbol(
                symbol,
                LAST_PRICES[symbol],
                trade_id=trade_id,
                order_id=order_id,
                oco_group=order_oco_group(ORDERS[order_id]),
            )
            if stop_fills:
    ...
    if action == "submit_limit":
        ...
        limit_fills = []
        if symbol in LAST_PRICES:
            limit_fills = evaluate_limit_fills_for_symbol(
                symbol,
                LAST_PRICES[symbol],
                trade_id=trade_id,
                order_id=order_id,
    ...
    # =========================
    # MANUAL REDUCE-ONLY EXIT LIMIT
    # =========================
    if action == "set_manual_exit_limit":
        if not trade_id:
            return jsonify({"ok": False, "error": "missing_trade_id", "message": "missing_trade_id"}), 400
        if not symbol:
        ...
        if limit_price is None:
            return jsonify({"ok": False, "error": "missing_limit_price", "message": "missing_limit_price"}), 400
        try:
            limit_price = float(limit_price)
        except (TypeError, ValueError):
        ...
        if not math.isfinite(limit_price):
            return jsonify({"ok": False, "error": "invalid_price", "message": "invalid_price"}), 400
        if not price_is_valid_tick(limit_price, symbol):
            return jsonify({
                "ok": False,
        ...
        position_symbol = find_position_symbol(symbol)
        position = POSITIONS.get(position_symbol) or {}
        position_qty = float(position.get("qty", 0) or 0)
        if position_qty == 0:
            return jsonify({"ok": False, "error": "position_flat_or_missing", "message": "position_flat_or_missing"}), 409

        requested_qty = float(data.get("qty", 0) or 0)
        if requested_qty <= 0:
            return jsonify({"ok": False, "error": "qty_must_be_positive", "message": "qty_must_be_positive"}), 400
        if requested_qty > abs(position_qty):
            return jsonify({
                "ok": False,
        ...
        oco_group = data.get("oco_group") or default_oco_group(trade_id)
        if not oco_group or oco_group != default_oco_group(trade_id):
            return jsonify({
                "ok": False,
        ...
        active_oco_peers = [
            order for order in active_orders_for_trade(trade_id)
            if order_oco_group(order) == oco_group and order.get("type") == "stop"
        ]
        if not active_oco_peers:
            return jsonify({
                "ok": False,
                "error": "oco_linkage_not_confirmed",
                "message": "oco_linkage_not_confirmed",
        ...
        active_limits = [
            order for order in active_orders_for_trade(trade_id, "limit")
            if str(order.get("symbol", "")).upper() == str(position_symbol).upper()
        ]
        existing_limits = [
            order for order in active_limits
            if order_oco_group(order) == oco_group
        ]
        if active_limits and len(active_limits) != len(existing_limits):
            return jsonify({
                "ok": False,
                "error": "oco_linkage_not_confirmed",
                "message": "active_limit_outside_protective_oco_scope",
            }), 409
        replace_existing_tp = data.get("replace_existing_tp") is True
        if existing_limits and not replace_existing_tp:
            return jsonify({
                "ok": False,
                "error": "active_limit_exists",
                "message": "active_limit_exists",
                "existing_limit_ids": [order["order_id"] for order in existing_limits],
            }), 409

        timestamp = datetime.now().isoformat()
        replaced_limit_ids = []
        if replace_existing_tp:
            for existing_limit in existing_limits:
                existing_limit["status"] = "cancelled"
                existing_limit["cancelled_at"] = timestamp
                existing_limit["closed_reason"] = "manual_exit_limit_replaced"
                replaced_limit_ids.append(existing_limit["order_id"])
        ...
        ORDERS[order_id] = {
            ...
            "level_label": str(data.get("level_label") or "").strip() or None,
            "replaced_limit_ids": replaced_limit_ids,
        }

        log(
            f"MANUAL EXIT LIMIT SET [{trade_id}] {position_symbol} "
            f"side={side} limit={limit_price} qty={requested_qty} replaced={replaced_limit_ids}"
        )

        limit_fills = []
        if position_symbol in LAST_PRICES:
            limit_fills = evaluate_limit_fills_for_symbol(
                position_symbol,
                LAST_PRICES[position_symbol],
                trade_id=trade_id,
                order_id=order_id,
                oco_group=order_oco_group(ORDERS[order_id]),
            )
            if limit_fills:
                log(f"MANUAL EXIT LIMIT FILLED IMMEDIATELY [{trade_id}] fills={limit_fills}")

        save_executor_state()

        return jsonify({
            "ok": True,
            "message": "Manual exit limit set",
            "broker_order_id": order_id,
            "order_id": order_id,
            "order": ORDERS[order_id],
            "replaced_limit_ids": replaced_limit_ids,
    ...
    if action == "modify_stop":
        ...
        if not broker_order_id:
            return jsonify({
                "ok": False,
                "error": "modify_stop_missing_order_id",
                "message": "modify_stop_missing_order_id",
            }), 400
        if stop_price is None:
            return jsonify({"ok": False, "message": "Missing stop_price"}), 400
        ...
        if not order:
            return jsonify({
                "ok": False,
                "error": "modify_stop_order_not_found",
                "message": "modify_stop_order_not_found",
            }), 404
        if trade_id and order.get("trade_id") != trade_id:
            return jsonify({
                "ok": False,
                "error": "modify_stop_trade_id_mismatch",
                "message": "modify_stop_trade_id_mismatch",
            }), 409
        if symbol and str(order.get("symbol", "")).upper() != symbol:
        ...
        log(
            f"STOP MODIFIED [{order.get('trade_id')}] {order.get('symbol')} order_id={broker_order_id} "
            f"stop={order['stop_price']} qty={order['qty']}"
        )

        stop_fills = []
        order_symbol = str(order.get("symbol", "")).upper()
        if order_symbol in LAST_PRICES:
            stop_fills = evaluate_stop_fills_for_symbol(
                order_symbol,
                LAST_PRICES[order_symbol],
                trade_id=order.get("trade_id"),
                order_id=broker_order_id,
                oco_group=order_oco_group(order),
            )
            if stop_fills:
                log(f"MODIFIED STOP FILLED IMMEDIATELY [{order.get('trade_id')}] fills={stop_fills}")
    ...
    if action == "cancel_order":
        ...
        if not trade_id or not broker_order_id:
            return jsonify({"ok": False, "message": "Missing trade_id or broker_order_id"}), 400
    ...
    if action == "flatten_symbol":
        ...
        if trade_id:
            ...
            scoped_trade_ids = active_trade_ids_for_symbol(symbol)
            if len(scoped_trade_ids) > 1:
                return ambiguous_symbol_scope_response(symbol, scoped_trade_ids)
            for order in active_orders_for_symbol(symbol):
                order["status"] = "closed"
                order["closed_at"] = timestamp
                order["closed_reason"] = "flatten_symbol"
        ...
        remaining_trade_ids = [
            scoped_trade_id
            for scoped_trade_id in active_trade_ids_for_symbol(symbol)
            if scoped_trade_id != trade_id
        ]
        if symbol in POSITIONS and not remaining_trade_ids:
            POSITIONS[symbol] = {
                "qty": 0.0,
                "avg_entry_price": 0.0,
                "updated_at": datetime.now().isoformat()
    ...
    if action == "move_stop_to_be":
        ...
        if active_stops:
            if len(active_stops) > 1:
                return jsonify({
                    ...
                    "error": "ambiguous_trade_scope",
                    "message": "ambiguous_trade_scope",
                    "existing_stop_ids": [order.get("order_id") for order in active_stops],
                }), 409
        ...
        if symbol in LAST_PRICES:
            stop_fills = evaluate_stop_fills_for_symbol(
                ...
                trade_id=trade_id,
                order_id=new_order_id,
                oco_group=order_oco_group(ORDERS[new_order_id]),
            )
            if stop_fills:
...
if __name__ == "__main__":
    load_executor_state()
    log_supported_executor_actions()
