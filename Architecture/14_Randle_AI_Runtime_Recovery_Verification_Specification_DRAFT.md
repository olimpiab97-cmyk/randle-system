...
The Phase 3B clause registry is historical rejected evidence and is headed `NOT APPROVAL READY — SEMANTIC TRACEABILITY REBUILD DEFERRED TO PHASE 3C2`. It is not a source of substantive completion. The verification IDs below remain stable family names for Phase 3C1-R1 schema/specification work, but no claim is made that every amended normative clause has a current scenario/assertion mapping. Phase 3C2 will rebuild semantic forward/reverse traceability only from hashes independently accepted in Phase 3C1-R1.
...
The control-store suite SHALL execute `docs/architecture/runtime_authority_store_schema_v2_DRAFT.sql` against a temporary database and prove the explanatory contract in `docs/architecture/runtime_authority_store_schema_DRAFT.md`: SQLite `3.43.1`; schema `RANDLE_RUNTIME_AUTHORITY_SCHEMA_V2`; `user_version=2`; valid STRICT declared types; 38 tables; 500 columns (440 nonnull/60 nullable); 38 primary keys/46 columns; every unique/check; 124 FK declarations/132 mappings with 124 explicit update/delete actions and 55 deferred/69 immediate declarations; `foreign_keys=ON`; zero `foreign_key_check` rows; 13 partial unique indexes; 14 compiled/exercised triggers; WAL/FULL durability; 60 active exclusive writer routes; exact external prepared-evidence sequence/hash binding for each store recovery row; reproducible schema/registry hashes; and all 55 operation IDs. It SHALL prove that only 52 are allowed in `transaction_commits` and that validate, quarantine, and version-conflict rejection cannot create a commit row. It SHALL prove that the Coordinator is mechanical only. No external database identity copy or cross-database FK is permitted.
...
Subscription tests SHALL require exact source producer/sequence, request/provider/evaluator/freshness/proof/integrity/transaction identities, accept matching NQ/session, and reject NQ/YM cross-reference, stale epoch, wrong bridge generation, missing acknowledgement/request, a syntactically valid provider acknowledgement that differs from the authenticated source event, and duplicate positive current result. Termination tests SHALL reject every arbitrary vocabulary token; prove five evidence links; distinguish `NONE` complete-absence proof from `UNKNOWN` uncertainty; and accept `BRIDGE_CRASH` only with authenticated process-exception or authoritative OS-handle crash evidence.
...
6. Phase 3C2 substantive bidirectional traceability complete against independently accepted Phase 3C1-R1 hashes;
