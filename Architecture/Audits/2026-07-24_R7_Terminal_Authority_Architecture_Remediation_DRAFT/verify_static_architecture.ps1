...
$unit2RequiredNegativeCaseIds=@(1..51|ForEach-Object{'U2BC-N'+$_.ToString('D3')})
$unit2MissingNegativeMutations=@($unit2RequiredNegativeMutations|Where-Object{$_ -cnotin $unit2NegativeMutations})
$unit2NegativeRegistryComplete=$unit2MissingNegativeMutations.Count-eq0 -and $unit2NegativeMutations.Count-eq$unit2RequiredNegativeMutations.Count -and $unit2NegativeMutations.Count-eq@($unit2NegativeMutations|Select-Object -Unique).Count -and ($unit2NegativeCaseIds-join"`n")-ceq($unit2RequiredNegativeCaseIds-join"`n")
...
Add-Check 'unit2-build-closure-verifier-and-negative-regressions-are-explicit' ($unit2NegativeRegistryComplete -and $unit2ClosureVerifierSource.Contains('COMPILER_INPUT_SET_MISMATCH') -and $unit2ClosureVerifierSource.Contains('COMPILER_INPUT_IDENTITY_MISMATCH') -and $unit2ClosureVerifierSource.Contains('COMPILER_INPUT_TYPE_MISMATCH') -and $unit2ClosureVerifierSource.Contains('COMPILER_ARGUMENT_VECTOR_MISMATCH') -and $unit2ClosureVerifierSource.Contains('GENERATED_SOURCE_TOKEN_INVALID') -and $unit2ClosureVerifierSource.Contains('PACKAGED_TOOL_IDENTITY_INVALID') -and $unit2ClosureVerifierSource.Contains('ExactJsonEqual') -and $unit2ClosureVerifierSource.Contains('ExactTreeBlobAuthority') -and $unit2ClosureVerifierSource.Contains('ReadGitBlobBytes') -and $unit2ClosureVerifierSource.Contains('GroundedConfigurationInventory') -and $unit2ClosureVerifierSource.Contains('RecomputeBuildInputClosure') -and $unit2ClosureVerifierSource.Contains('GroundedPolicyAuthority') -and $unit2ClosureVerifierSource.Contains('AssertGeneratedOutputAuthority') -and $unit2ClosureVerifierSource.Contains('GroundedGeneratorAuthority') -and $unit2ClosureVerifierSource.Contains('GeneratedSourceContract') -and $unit2ClosureVerifierSource.Contains('AssertDeterminismCompilerInputs') -and $unit2ClosureVerifierSource.Contains('AssertCompilerInputBinding') -and $unit2ClosureVerifierSource.Contains('ExtractIdentity') -and $unit2ClosureVerifierSource.Contains('ValidateModel')) ([ordered]@{missing_mutations=$unit2MissingNegativeMutations;negative_case_count=@($unit2Negative.cases).Count;negative_registry_sha256=(Get-LowerHash $unit2NegativePath);verifier_sha256=(Get-LowerHash $unit2ClosureVerifierPath)})
