...
Add-Check 'unit2b-installer-is-stopped-prestart-only-and-rollback-bound' ($unit2bForbidden.Count -eq 0 -and $failureActionLegacyFindings.Count -eq 0 -and $unit2bInstaller.Contains('[Parameter(Mandatory=$true)][switch]$PreStartOnly') -and $unit2bInstaller.Contains('Only the explicit PreStartOnly path is exposed') -and $unit2bInstaller.Contains("Run `$artifactTool @('capture-failure-actions'") -and $unit2bInstaller.Contains("Run `$artifactTool @('configure-failure-actions-none'") -and $unit2bInstaller.Contains("Run `$artifactTool @('verify-failure-actions-none'") -and $unit2bInstaller.Contains("Run `$artifactTool @('restore-failure-actions'") -and $unit2bInstaller.Contains('Run $artifactTool @(''service-boundary''') -and $unit2bInstaller.Contains('Run $artifactTool @(''restore-service-boundary''') -and $unit2bInstaller.Contains("`$aclSnapshots.Add") -and $unit2bInstaller.Contains('Set-Acl -LiteralPath') -and $unit2bInstaller.Contains("'ROLLBACK_SCM_CONFIG'") -and $unit2bInstaller.Contains("'ROLLBACK_SCM_SID_TYPE'") -and $unit2bInstaller.Contains("'ROLLBACK_SCM_PRIVILEGES'") -and $unit2bInstaller.Contains('$failureActionsRestoreRequired=$true') -and $unit2bInstaller.Contains('service_started=$false') -and $unit2bInstaller.Contains('terminal_transition_authorized=$false')) ([ordered]@{failure_action_legacy_findings=$failureActionLegacyFindings;forbidden_tokens=$unit2bForbidden.ToArray();installer_sha256=(Get-LowerHash $unit2bInstallerPath)})
