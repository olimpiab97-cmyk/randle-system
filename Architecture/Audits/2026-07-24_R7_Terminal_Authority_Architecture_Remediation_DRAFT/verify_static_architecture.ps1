...
$requiredDocumentationTokens=@('terminal signer, execution, observation, comparator, and upgrade authority','SeDenyInteractiveLogonRight','SeDenyRemoteInteractiveLogonRight','POS-005','POS-006','canonical','R7MeasuredUtility.cs','R7RecoveryProbeAuditor','runtime dependency closure','matrix','No live outer execution occurred','No upgrade transition was attempted')
$missingDocumentationTokens=@($requiredDocumentationTokens|Where-Object{-not $documentationCorpus.Contains($_)})
$boundedDocumentationClaims=$readmeText.Contains('Every `R7AR-B01` through `R7AR-B16` finding remains **PARTIAL**') -and $readmeText.Contains('None of these services, rights, SIDs, ACLs, or denials was created or changed in this static unit') -and $aiaText.Contains('All sixteen independent-review blockers remain partial') -and $aiaText.Contains('These controls are source design, not current host fact') -and $deltaText.Contains('The present unit creates static source and verification evidence only') -and $deltaText.Contains('It creates no canonical record, host transition, principal, key, trust change, ledger entry, receipt, reconciliation, matrix run, deployment, or review disposition') -and $threatText.Contains('All `R7AR-B01` through `R7AR-B16` findings therefore remain partial') -and $threatText.Contains('does **not** establish that any proposed service')
Add-Check 'proposal-readme-threat-and-blocker-documentation-synchronized-to-static-only-partial-state' ($missingDocumentationTokens.Count -eq 0 -and $boundedDocumentationClaims) ([ordered]@{architecture_impact_assessment_sha256=(Get-LowerHash $aiaPath);canonical_delta_sha256=(Get-LowerHash $deltaPath);missing_tokens=$missingDocumentationTokens;readme_sha256=(Get-LowerHash $readmePath);threat_model_sha256=(Get-LowerHash $threatPath)})
