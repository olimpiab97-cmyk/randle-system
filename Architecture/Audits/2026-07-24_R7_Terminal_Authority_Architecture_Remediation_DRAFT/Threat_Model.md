...
Bootstrap failure is not authority. The preserved exit-1639 attempt stopped before service/key/certificate creation; the preserved CNG-compatibility attempt left only a stopped restricted service entry and created no certificate or key file. A third attempt used CNG `KeySpec None` and created one nonexportable key and public certificate but failed before bootstrap completion when the public certificate file acquired an empty protected DACL. It installed no binary, policy, ledger, or authorization and never started the service. The corrected bootstrap requires and propagates all three exact failed-attempt identities, verifies that stopped certificate/key state without reading private bytes, restores only the public-file ACL with a content-bound owner-recovery utility, and replaces the key-file Administrator grant with metadata-only rights. None of the interruptions can be omitted or reinterpreted as provisioning success.
