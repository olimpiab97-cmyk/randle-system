...
Bootstrap failure is not authority. The preserved exit-1639 attempt stopped before service/key/certificate creation; the preserved CNG-compatibility attempt left only a stopped restricted service entry and created no certificate or key file. The corrected bootstrap requires and propagates both exact failed-attempt identities, verifies that stopped configuration before resuming, and uses CNG `KeySpec None`, so neither interruption can be omitted or reinterpreted as provisioning success.
