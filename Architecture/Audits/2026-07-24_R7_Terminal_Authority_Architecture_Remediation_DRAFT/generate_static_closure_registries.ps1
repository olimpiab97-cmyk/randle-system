...
if ($GenerateRegistries) {
    $scriptDefinitions = @(
        ...
        (New-ScriptDefinition 'build_remediation_package.ps1' 'FUTURE_TRANSITION_BUILD' @('FUTURE_POSTCOMMIT_BUILD') 'BUILD_TIME' @('POWERSHELL_ORCHESTRATOR','CSC_COMPILER','ILDASM_TOOL','GIT_BUILD_AND_VERIFICATION','ARTIFACT_TOOL_BUILD_OUTPUT') 'FUTURE_HOST_TRANSITION_INPUT_NOT_INVOKED_IN_STATIC_UNIT'),
        ...
        (New-ScriptDefinition 'provision_upgrade_authority.ps1' 'FUTURE_UPGRADE_BOOTSTRAP' @('FUTURE_SEPARATELY_AUTHORIZED_HOST_TRANSITION') 'INSTALLATION_TIME' @('POWERSHELL_ORCHESTRATOR','SC_SERVICE_CONTROL_TOOL','ICACLS_ACL_TOOL','POWERSHELL_MANAGEMENT_ASSEMBLY','PKI_MODULE_MANIFEST') 'FUTURE_KEY_AND_SERVICE_PLAN_NOT_INVOKED_IN_STATIC_UNIT'),
    ...
    $utilityDefinitions = @(
        (New-UtilityDefinition 'GIT_BUILD_AND_VERIFICATION' 'C:\Program Files\Git\cmd\git.exe' @('BUILD_TIME','VERIFICATION_TIME','FUTURE_MATRIX') @('extract_immutable_authority.ps1','build_remediation_package.ps1','build_static_closure.ps1','capture_remediation_host_state.ps1','run_fresh_matrix.ps1','scan_secrets_and_contamination.ps1','verify_static_architecture.ps1') 'ABSOLUTE_PATH_ONLY; ENVIRONMENT_AND_CONFIG_CLOSED; PROHIBITED_FROM_RUNTIME_AUTHORITY' 'NONAUTHORITATIVE_BUILD_AND_VERIFICATION_TOOL' 'RECURSIVE_INSTALLATION_ROOT' @('cat-file','ls-tree','rev-parse','status','diff','clone','checkout')),
        (New-UtilityDefinition 'POWERSHELL_ORCHESTRATOR' 'C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe' @('BUILD_TIME','VERIFICATION_TIME','FUTURE_INSTALLATION','FUTURE_MATRIX') @($actualScriptNames) 'ABSOLUTE_HOST_PATH; NONAUTHORITATIVE_ORCHESTRATOR; SCRIPT_BYTES_SEPARATELY_GOVERNED' 'NONAUTHORITATIVE_ORCHESTRATOR' 'RECURSIVE_POWERSHELL_ROOT' @('PowerShell-script-host')),
        (New-UtilityDefinition 'CSC_COMPILER' 'C:\Windows\Microsoft.NET\Framework64\v4.0.30319\csc.exe' @('BUILD_TIME') @('build_remediation_package.ps1','build_static_closure.ps1') 'ABSOLUTE_PATH; FIXED OPTIONS; NO RESPONSE FILES; NO PATH SEARCH' 'NONAUTHORITATIVE_COMPILER_INPUT' 'RECURSIVE_FRAMEWORK_ROOT' @('compile-x64-net48')),
        (New-UtilityDefinition 'ILDASM_TOOL' 'C:\Program Files (x86)\Microsoft SDKs\Windows\v10.0A\bin\NETFX 4.8 Tools\x64\ildasm.exe' @('BUILD_TIME') @('build_remediation_package.ps1','build_static_closure.ps1') 'ABSOLUTE_PATH; NORMALIZED_IL_COMPARISON_ONLY' 'NONAUTHORITATIVE_BINARY_SEMANTIC_CHECK_TOOL' 'RECURSIVE_ILDASM_ROOT' @('normalized-il')),
        (New-UtilityDefinition 'SC_SERVICE_CONTROL_TOOL' 'C:\Windows\System32\sc.exe' @('READ_ONLY_PREFLIGHT','FUTURE_INSTALLATION') @('capture_remediation_host_state.ps1','install_authorized_transition.ps1','provision_upgrade_authority.ps1') 'ABSOLUTE_PATH; STATIC_UNIT_READ_ONLY_QUERY ONLY; MUTATION DEFERRED' 'FUTURE_HOST_TRANSITION_UTILITY_NOT_INVOKED_FOR_MUTATION' 'FILE_MEASUREMENT_OS_TCB' @('qc','qsidtype','qprivs','future-create-config-delete')),
        (New-UtilityDefinition 'ICACLS_ACL_TOOL' 'C:\Windows\System32\icacls.exe' @('FUTURE_INSTALLATION','FUTURE_MATRIX_FIXTURE') @('install_authorized_transition.ps1','provision_upgrade_authority.ps1','run_fresh_matrix.ps1') 'ABSOLUTE_PATH; NOT INVOKED IN STATIC UNIT; FUTURE HELD-MEASUREMENT WRAPPER REQUIRED' 'FUTURE_HOST_OR_FIXTURE_MUTATION_UTILITY_NOT_INVOKED' 'FILE_MEASUREMENT_OS_TCB' @('future-acl-application')),
        ...
        (New-UtilityDefinition 'RUNTIME_MACHINE_CONFIG' 'C:\Windows\Microsoft.NET\Framework64\v4.0.30319\Config\machine.config' @('BUILD_TIME','RUNTIME_INPUT') @('build_remediation_package.ps1','build_static_closure.ps1') 'CONTENT_BOUND_CONFIGURATION; NO ENVIRONMENT OVERRIDE' 'FRAMEWORK_CONFIGURATION_INPUT' 'RECURSIVE_FRAMEWORK_ROOT' @('clr-configuration')),
        (New-UtilityDefinition 'COMPILER_REFERENCE_mscorlib.dll' 'C:\Program Files (x86)\Reference Assemblies\Microsoft\Framework\.NETFramework\v4.8\mscorlib.dll' @('BUILD_TIME') @('build_remediation_package.ps1','build_static_closure.ps1') 'ABSOLUTE_REFERENCE; NOSTDLIB; EXPLICIT REFERENCE' 'COMPILER_REFERENCE_INPUT' 'RECURSIVE_REFERENCE_ROOT' @('compile-reference')),
        ...
        (New-UtilityDefinition 'POWERSHELL_MANAGEMENT_ASSEMBLY' $managementAssembly @('READ_ONLY_PREFLIGHT','FUTURE_INSTALLATION','FUTURE_MATRIX') @('capture_remediation_host_state.ps1','install_authorized_transition.ps1','provision_upgrade_authority.ps1','run_fresh_matrix.ps1') 'CMDLETS RESOLVE TO THIS MEASURED ASSEMBLY; HOST MUTATION DEFERRED' 'POWERSHELL_CMDLET_PROVIDER_INPUT' 'FILE_MEASUREMENT_OS_TCB' @('Get-Service','Start-Service','Stop-Service','New-Item')),
        ...
        (New-UtilityDefinition 'PKI_MODULE_MANIFEST' 'C:\Windows\System32\WindowsPowerShell\v1.0\Modules\PKI\pki.psd1' @('FUTURE_UPGRADE_PROVISIONING') @('provision_upgrade_authority.ps1') 'EXPLICIT IMPORT BY ABSOLUTE MANIFEST; NOT IMPORTED IN STATIC UNIT' 'FUTURE_KEY_PROVISIONING_MODULE_NOT_INVOKED' 'RECURSIVE_POWERSHELL_ROOT' @('New-SelfSignedCertificate','Export-Certificate')),
        (New-UtilityDefinition 'PKI_MODULE_TYPES' 'C:\Windows\System32\WindowsPowerShell\v1.0\Modules\PKI\pki.types.ps1xml' @('FUTURE_UPGRADE_PROVISIONING') @('provision_upgrade_authority.ps1') 'CONTENT BOUND MODULE INPUT; NOT LOADED IN STATIC UNIT' 'FUTURE_KEY_PROVISIONING_MODULE_NOT_INVOKED' 'RECURSIVE_POWERSHELL_ROOT' @('PKI-types'))
    )
    if (-not (Test-Path -LiteralPath $measurementRoot)) { New-Item -ItemType Directory -Path $measurementRoot | Out-Null }
    $utilityIndex = 0
    ...
    $targets = @(
        [ordered]@{authority_classification='FUTURE_UPGRADE_AUTHORITY_BINARY';define='';file_name='RandleTerminalUpgradeAuthority.exe';installed_role=$true;main='RandleAI.R7Remediation.R7UpgradeServiceProgram';role='UPGRADE_AUTHORITY'},
        [ordered]@{authority_classification='NONAUTHORITATIVE_UPGRADE_CLIENT';define='';file_name='RandleTerminalUpgradeClient.exe';installed_role=$true;main='RandleAI.R7Remediation.R7UpgradeClientProgram';role='UPGRADE_CLIENT'},
    ...
    $sourceDefinitions = @(
        ...
        (New-SourceDefinition 'R7BuildClosureVerifier.cs' @('BUILD_AND_BINARY_CLOSURE_VERIFIER') @('R7AR-B07','R7AR-B12','R7AR-B15') @('DEPENDENCY_VERIFIER','SOURCE_AND_RUNTIME') @('STATIC_BUILD_RECEIPT','STATIC_TRACE') 'DISPOSITION_DETERMINATIVE_WHEN_AUTHORIZED_AND_INSTALLED' @('TERMINAL_SIGNER','UPGRADE_AUTHORITY','PUBLIC_VERIFIER')),
        ...
        (New-SourceDefinition 'R7CryptoLedger.cs' @('CRYPTOGRAPHIC_LEDGER','DURABLE_FILE') @('R7AR-B08','R7AR-B09','R7AR-B10') @('TERMINAL_PIPE','RECOVERY_ENGINE','PUBLIC_VERIFIER','UPGRADE_PIPE') @('STATIC_TRANSACTION','STATIC_RECOVERY','STATIC_HISTORY','STATIC_TRACE') 'DISPOSITION_DETERMINATIVE_WHEN_AUTHORIZED_AND_INSTALLED' @('TERMINAL_SIGNER','UPGRADE_AUTHORITY','PUBLIC_VERIFIER')),
        ...
        (New-SourceDefinition 'R7Policy.cs' @('POLICY_AND_VERSION_BINDING') @('R7AR-B07','R7AR-B08','R7AR-B12') @('UPGRADE_PIPE','TERMINAL_PIPE','PUBLIC_VERIFIER') @('COMPILE','STATIC_TRACE','FUTURE_ACTIVATION') 'DISPOSITION_DETERMINATIVE_WHEN_AUTHORIZED_AND_INSTALLED' @('TERMINAL_SIGNER','UPGRADE_AUTHORITY','PUBLIC_VERIFIER')),
        ...
        (New-SourceDefinition 'R7ServiceInfrastructure.cs' @('NAMED_PIPE_SERVER','OS_CALLER_CAPTURE') @('R7AR-B05','R7AR-B06') @('TERMINAL_PIPE','OS_BOUNDARY') @('COMPILE','PARSER_22_CASES','STATIC_TRACE','FUTURE_LIVE_IPC') 'DISPOSITION_DETERMINATIVE_WHEN_AUTHORIZED_AND_INSTALLED' @('TERMINAL_SIGNER','UPGRADE_AUTHORITY','EXECUTION','OBSERVATION','COMPARATOR')),
        ...
        (New-SourceDefinition 'R7Transaction.cs' @('APPEND_ONLY_TRANSACTION_STATE_MACHINE') @('R7AR-B09','R7AR-B10') @('TERMINAL_PIPE','RECOVERY_ENGINE') @('STATIC_TRANSACTION','STATIC_RECOVERY','COMPILE','STATIC_TRACE') 'DISPOSITION_DETERMINATIVE_WHEN_AUTHORIZED_AND_INSTALLED' @('TERMINAL_SIGNER','UPGRADE_AUTHORITY')),
        (New-SourceDefinition 'R7UpgradeAuthorityService.cs' @('SEPARATE_NONCIRCULAR_UPGRADE_AUTHORITY') @('R7AR-B07') @('UPGRADE_PIPE') @('COMPILE','STATIC_TRACE','FUTURE_PREINSTALL_AUTHORIZATION_ATTACKS') 'UPGRADE_DISPOSITION_DETERMINATIVE_WHEN_SEPARATELY_PROVISIONED' @('UPGRADE_AUTHORITY')),
        (New-SourceDefinition 'R7UpgradeClient.cs' @('UPGRADE_AUTHORITY_CLIENT') @('R7AR-B07') @('UPGRADE_PIPE') @('COMPILE','STATIC_TRACE','FUTURE_PREINSTALL_AUTHORIZATION') 'NONAUTHORITATIVE_SUPPORT' @('UPGRADE_CLIENT'))
    )
