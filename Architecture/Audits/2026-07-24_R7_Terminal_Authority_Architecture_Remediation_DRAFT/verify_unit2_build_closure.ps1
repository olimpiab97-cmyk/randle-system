...
public static class R7GitBatchProcessRunner {
    ...
    const long MaximumObjectBytes=67108864L, MaximumAggregateBytes=268435456L, N154ObservationGuardMilliseconds=2L, N156ObservationLeadMilliseconds=50L, N156ReconciliationGuardMilliseconds=5L;
    ...
    static void CleanupCreatedProcessAndFail(string primaryCode,string primaryDetail,string fault,ref PROCESS_INFORMATION pi,ref IntPtr job,Stopwatch clock,int deadline,int confirmationMilliseconds,
        ...
        bool injectParentWait=fault!=null&&fault.IndexOf("PARENT_WAIT_FAIL",StringComparison.Ordinal)>=0;bool injectFinalQuery=fault!=null&&(fault.IndexOf("FINAL_QUERY_FAIL",StringComparison.Ordinal)>=0||fault.IndexOf("CONFIRM_FAIL",StringComparison.Ordinal)>=0);bool forceFallback=fault!=null&&fault.IndexOf("X01_FALLBACK",StringComparison.Ordinal)>=0;bool n154PostDeadlineGate=forceFallback&&fault!=null&&fault.IndexOf("X01_N154_POST_DEADLINE_GATE",StringComparison.Ordinal)>=0;bool n156CrossDeadlineGate=fault!=null&&fault.IndexOf("X01_PID_RECONCILIATION_AFTER_DEADLINE",StringComparison.Ordinal)>=0&&fault.IndexOf("X01_N156_CROSS_DEADLINE_GATE",StringComparison.Ordinal)>=0;long n154GuardTicks=MillisecondsToStopwatchTicks(N154ObservationGuardMilliseconds),n154ReleaseTicks=0,n156LeadTicks=MillisecondsToStopwatchTicks(N156ObservationLeadMilliseconds),n156GuardTicks=MillisecondsToStopwatchTicks(N156ReconciliationGuardMilliseconds),n156EntryTicks=0,n156ReleaseTicks=0;bool n154GateApplied=false,n156GateApplied=false;CleanupSnapshot primary=null,primaryDiagnostic=null,final=null;
        ...
        if(primary==null)primary=UnmeasuredSnapshot(clock,deadline,(int)primaryEnd,primaryEndTicks,inputTask,stdoutTask,stderrTask,"PrimaryFallback",primaryStart);bool primaryConfirmed=primary.PrimaryEligible;
        ...
        if(!finalConfirmed)Fail("GIT_BATCH_PROCESS_TREE_TERMINATION_FAILED",detail);bool preserveStimulus=primaryCode=="GIT_BATCH_STIMULUS_NOT_READY";if(!primaryConfirmed&&!preserveStimulus)Fail("GIT_BATCH_PROCESS_TREE_TERMINATION_FAILED","reason=primary-confirmation-deadline;"+detail);Fail(primaryCode,detail);
    }
