...
public static class R7GitBatchProcessRunner {
    ...
    static CleanupSnapshot CaptureSnapshot(IntPtr process,IntPtr job,Stopwatch clock,int totalDeadline,int observationDeadline,Task inputTask,Task stdoutTask,Task stderrTask,string phase,long phaseStart,bool diagnosticOnly,bool injectWaitFailure,bool injectQueryFailure,out int waitError,out int queryError){
        ...
        return CanonicalSnapshot(parent,query,active,input,stdout,stderr,Math.Max(0,completed-phaseStart),Math.Max(0,totalDeadline-completed),observationStarted,completed,observationDeadline,eligible,diagnosticOnly,false,false,false);
    }
    static CleanupSnapshot UnmeasuredSnapshot(Stopwatch clock,int totalDeadline,int observationDeadline,Task inputTask,Task stdoutTask,Task stderrTask,string phase,long phaseStart){long started=clock.ElapsedMilliseconds;TaskObservation input=ObserveTask(inputTask,clock,phase),stdout=ObserveTask(stdoutTask,clock,phase),stderr=ObserveTask(stderrTask,clock,phase);long completed=clock.ElapsedMilliseconds;return CanonicalSnapshot(false,"NotAttempted",UInt32.MaxValue,input,stdout,stderr,Math.Max(0,completed-phaseStart),Math.Max(0,totalDeadline-completed),started,completed,observationDeadline,false,true,false,false,false);}
    ...
    static void CleanupCreatedProcessAndFail(string primaryCode,string primaryDetail,string fault,ref PROCESS_INFORMATION pi,ref IntPtr job,Stopwatch clock,int deadline,int confirmationMilliseconds,
        ...
        if(primary==null)primary=UnmeasuredSnapshot(clock,deadline,(int)primaryEnd,inputTask,stdoutTask,stderrTask,"PrimaryFallback",primaryStart);bool primaryConfirmed=primary.PrimaryEligible;
        ...
        long finalStart=clock.ElapsedMilliseconds;while(clock.ElapsedMilliseconds<=terminalCutoff){int waitError,queryError;CleanupSnapshot candidate=CaptureSnapshot(pi.hProcess,job,clock,deadline,(int)terminalCutoff,inputTask,stdoutTask,stderrTask,"Final",finalStart,false,false,injectFinalQuery,out waitError,out queryError);if(finalWaitError==0)finalWaitError=waitError;if(finalQueryError==0)finalQueryError=queryError;final=candidate;bool absent=PidsAbsent(pidList.ToArray());if(candidate.PrimaryEligible&&absent){final=WithExternalDisposition(candidate,clock,deadline,true);break;}if(clock.ElapsedMilliseconds<terminalCutoff)Thread.Sleep(1);}
...
function InvokeGitXAuthorityFixture([string]$Mutation) {
    ...
    if($Mutation-like'X01_*'){$confirmation=1;$fault='POST_TASK_FORCED_CLEANUP_PRIMARY_LATE_PROCESS_TRANSITION_'+$Mutation;if($Mutation-in@('X01_FALLBACK_AFTER_DEADLINE','X01_FALLBACK_BEFORE_TASK_OBSERVATION')){$fault+='_X01_FALLBACK'};if($Mutation-eq'X01_UNKNOWN_JOB_AUTHORITY'){$fault+='_FINAL_QUERY_FAIL'};if($Mutation-eq'X01_POST_JOB_CLOSE_SNAPSHOT'){$fault+='_FORCE_JOB_CLOSE'}}
