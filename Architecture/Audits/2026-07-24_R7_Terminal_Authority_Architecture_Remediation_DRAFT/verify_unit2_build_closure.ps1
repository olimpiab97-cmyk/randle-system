...
function FieldValue([object]$Value,[string]$Name) { if($Value -is [Collections.IDictionary]){return $Value[$Name]};return $Value.PSObject.Properties[$Name].Value }
...
function ValidateModel([object]$Receipt,[hashtable]$GeneratedTextOverrides) {
    $declaredSourcePaths=@($Receipt.source_files|ForEach-Object{[string]$_.path}|Sort-Object)
    ...
    foreach($role in @($Receipt.roles)){
        ...
        if($generatedInput.Count-ne1){Fail 'COMPILER_INPUT_SET_MISMATCH' ([string]$role.role)}
        ...
        $expectedInputs=@($declaredSourcePaths + [string]$generatedForRole[0].path | Sort-Object)
        if((@($inputPaths|Sort-Object)-join "`n") -cne ($expectedInputs-join "`n")){Fail 'COMPILER_INPUT_SET_MISMATCH' ([string]$role.role)}
...
$expectedSourceRows=@($expectedSourceRows|Sort-Object{[string](FieldValue $_ 'path')})
$receiptSourceRows=@($receipt.source_files|Sort-Object path)
...
$negativeRegistry=ReadJson $negativePath;$negativeResults=[Collections.Generic.List[object]]::new()
