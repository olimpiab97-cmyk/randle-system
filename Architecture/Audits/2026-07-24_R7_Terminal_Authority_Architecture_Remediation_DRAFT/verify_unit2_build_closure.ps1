...
public static class R7GitBatchProcessRunner {
    ...
    static void CleanupCreatedProcessAndFail(string primaryCode,string primaryDetail,string fault,ref PROCESS_INFORMATION pi,ref IntPtr job,Stopwatch clock,int deadline,int confirmationMilliseconds,
        ...
        Close(ref pi.hThread);Close(ref stdinRead);Close(ref stdoutWrite);Close(ref stderrWrite);inputWorker.Cancel();stdoutWorker.Cancel();stderrWorker.Cancel();try{if(stdin!=null)stdin.Dispose();}catch{}Close(ref stdinWrite);
        ...
        if(final.Input.State=="Running"||final.Stdout.State=="Running"||final.Stderr.State=="Running"||!externalAbsent||!final.ParentSignaled)Fail("GIT_BATCH_PROCESS_TREE_TERMINATION_FAILED","reason=terminalization-unconfirmed;"+detail);
        if(!finalConfirmed)Fail("GIT_BATCH_PROCESS_TREE_TERMINATION_FAILED",detail);bool preserveStimulus=primaryCode=="GIT_BATCH_STIMULUS_NOT_READY"||primaryCode=="GIT_BATCH_PRIMARY_ACTIVITY_STIMULUS_FAILED";if(!primaryConfirmed&&!preserveStimulus)Fail("GIT_BATCH_PROCESS_TREE_TERMINATION_FAILED","reason=primary-confirmation-deadline;"+detail);Fail(primaryCode,detail);
    }
    ...
    static R7GitProcessResult RunCore(string executable,string arguments,string workingDirectory,byte[] standardInput,R7GitExpectedObject[] expected,int totalDeadlineMilliseconds,int cleanupReserveMilliseconds,int terminationConfirmationMilliseconds,int stderrLimit,string testFault,string readinessPath,int readinessCount,string readinessNonce,string readinessCase){
        ...
        try{
            ...
            stdoutTask=Task.Factory.StartNew<R7GitAuthenticatedObject[]>(delegate{stdoutWorker.Attach();if(n186Gate)n186StdoutActivated.Set();try{if(readinessGated||n149ActivityGate){byte[] proofBytes=DrainReadinessProof(outLocal,4096,"stdout",readinessProof);Interlocked.Exchange(ref observedStdout,proofBytes.LongLength);return new R7GitAuthenticatedObject[0];}return Parse(outLocal,expected,aggregate,delegate(long value){Interlocked.Exchange(ref observedStdout,value);});}finally{stdoutWorker.Detach();}},CancellationToken.None,TaskCreationOptions.LongRunning,TaskScheduler.Default);
            n186StdoutTaskCreatedTicks=clock.ElapsedTicks;
            stderrTask=Task.Factory.StartNew<StderrState>(delegate{stderrWorker.Attach();try{if(readinessGated||n149ActivityGate){byte[] proofBytes=DrainReadinessProof(errLocal,Math.Min(stderrLimit,4096),"stderr",readinessProof);return new StderrState{Bytes=proofBytes,Total=proofBytes.LongLength};}return DrainStderr(errLocal,stderrLimit);}finally{stderrWorker.Detach();}},CancellationToken.None,TaskCreationOptions.LongRunning,TaskScheduler.Default);
            inputCancellation=new CancellationTokenSource();if(testFault=="POST_TASK_INPUT_CANCELED")inputCancellation.Cancel();CancellationToken inputToken=inputCancellation.Token;inputTask=Task.Factory.StartNew(delegate{inputWorker.Attach();try{if(testFault=="POST_TASK_INPUT_FAULT")throw new InvalidDataException("GIT_BATCH_INPUT_TASK_FAULT|stage=input-task");if(standardInput!=null&&standardInput.Length!=0)inLocal.Write(standardInput,0,standardInput.Length);inLocal.Flush();inLocal.Dispose();}finally{inputWorker.Detach();}},inputToken,TaskCreationOptions.LongRunning,TaskScheduler.Default);
...
function InvokeGitXAuthorityFixture([string]$Mutation) {
    InitializeGitBatchProcessRuntime;$exe=(Get-Process -Id $PID).Path;$deadline=2500;$reserve=700;$confirmation=100;$code='Start-Sleep -Seconds 20';$fault='';$expected=@();$readinessPath='';$readinessCount=0;$readinessNonce='';$readinessCase=''
    ...
    elseif($Mutation-like'X03_*'){
        ...
        switch($Mutation){
            ...
            'X03_STDERR_RUNNING_UNTIL_CLOSE'{$code='[Console]::Out.Close();Start-Sleep -Seconds 20'}
            {$_-in@('X03_BOTH_RUNNING_UNTIL_JOB_CLOSE','X03_DESCENDANT_EXITS_AFTER_JOB_CLOSE','X03_PID_DISAPPEARS_AFTER_JOB_CLOSE')}{$fault+='_FORCE_JOB_CLOSE';break}
            'X03_INPUT_FAULTS_DURING_TERMINALIZATION'{$fault='POST_TASK_INPUT_FAULT'}
            'X03_INPUT_CANCELS_DURING_TERMINALIZATION'{$fault='POST_TASK_INPUT_CANCELED'}
            'X03_STDOUT_FAULTS_DURING_TERMINALIZATION'{$code='[Console]::Out.WriteLine(''JUNK'');[Console]::Out.Flush();Start-Sleep -Seconds 20';$fault='POST_TASK_FORCED_CLEANUP_'+$Mutation}
            'X03_PARENT_EXITS_WITH_ACTIVE_PIPES'{$code='$p=[Diagnostics.Process]::Start('''+$exe.Replace("'","''")+''',''-NoProfile -NonInteractive -EncodedCommand '+(EncodeLifecycleCommand 'Start-Sleep -Seconds 20')+''');exit 0'}
    ...
    $arguments='-NoProfile -NonInteractive -EncodedCommand '+(EncodeLifecycleCommand $code);if($Mutation-eq'X03_STDOUT_RUNNING_UNTIL_CLOSE'){$readinessPath='n186-fixture:'+(TextHash ($exe+"`n"+$arguments))};$clock=[Diagnostics.Stopwatch]::StartNew()
