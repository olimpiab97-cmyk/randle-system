...
public static class R7GitBatchProcessRunner {
    ...
    static IntPtr BuildAttributeList(IntPtr stdinRead,IntPtr stdoutWrite,IntPtr stderrWrite,IntPtr job,string fault,out IntPtr handleValues,out IntPtr jobValue){
        ...
        if(bytes.ToUInt64()>Int32.MaxValue)Fail("GIT_BATCH_ATTRIBUTE_LIST_INITIALIZATION_FAILED","stage=size;reason=overflow");IntPtr list=Marshal.AllocHGlobal((int)bytes.ToUInt64());handleValues=IntPtr.Zero;jobValue=IntPtr.Zero;
        ...
        try{
            ...
            initialized=true;handleValues=Marshal.AllocHGlobal(IntPtr.Size*3);Marshal.WriteIntPtr(handleValues,0,stdinRead);Marshal.WriteIntPtr(handleValues,IntPtr.Size,stdoutWrite);Marshal.WriteIntPtr(handleValues,IntPtr.Size*2,stderrWrite);
            ...
            if(jobValue!=IntPtr.Zero){Marshal.FreeHGlobal(jobValue);jobValue=IntPtr.Zero;}if(handleValues!=IntPtr.Zero){Marshal.FreeHGlobal(handleValues);handleValues=IntPtr.Zero;}if(initialized)DeleteProcThreadAttributeList(list);Marshal.FreeHGlobal(list);throw;
        }
    }
    ...
    static void CleanupCreatedProcessAndFail(string primaryCode,string primaryDetail,string fault,ref PROCESS_INFORMATION pi,IntPtr job,Stopwatch clock,int deadline,
        ...
        FileStream stdin,FileStream stdout,FileStream stderr,Task inputTask,Task stdoutTask,Task stderrTask,bool resumed){
        Stopwatch cleanup=Stopwatch.StartNew();bool jobTerminated=false,processTerminated=false,parentSignaled=false,activeKnown=false;uint active=UInt32.MaxValue;int jobError=0,processError=0,queryError=0,waitError=0;bool authorityFailure=false;
        ...
        IntPtr jobTerminationHandle=(injectJobTerminate||injectBoth)?IntPtr.Zero:job;
        jobTerminated=TerminateJobObject(jobTerminationHandle,230);jobError=jobTerminated?0:Marshal.GetLastWin32Error();
        uint initialWait=WaitForSingleObject(pi.hProcess,0);bool forceProcess=injectProcessTerminate||injectBoth;
        if(initialWait!=WAIT_OBJECT_0||forceProcess){IntPtr processTerminationHandle=forceProcess?IntPtr.Zero:pi.hProcess;processTerminated=TerminateProcess(processTerminationHandle,231);processError=processTerminated?0:Marshal.GetLastWin32Error();}
        if(injectBoth){
            bool retry=TerminateJobObject(job,232);int retryError=retry?0:Marshal.GetLastWin32Error();jobTerminated=retry;primaryDetail+=";job_retry="+retry+";job_retry_win32="+retryError.ToString(CultureInfo.InvariantCulture);
        }
        if(fault!=null&&fault.IndexOf("PARENT_WAIT_FAIL",StringComparison.Ordinal)>=0){uint failedWait=WaitForSingleObject(IntPtr.Zero,0);waitError=failedWait==WAIT_FAILED?Marshal.GetLastWin32Error():0;authorityFailure=true;}
        ...
        if(fault!=null&&fault.IndexOf("CLEANUP_DEADLINE",StringComparison.Ordinal)>=0){int remaining=(int)Math.Max(0,deadline-clock.ElapsedMilliseconds);if(remaining>0)Thread.Sleep(remaining);authorityFailure=true;}
        while(clock.ElapsedMilliseconds<deadline){
            uint wait=WaitForSingleObject(pi.hProcess,0);if(wait==WAIT_OBJECT_0)parentSignaled=true;else if(wait==WAIT_FAILED){waitError=Marshal.GetLastWin32Error();authorityFailure=true;}
            bool injectQuery=fault!=null&&(fault.IndexOf("FINAL_QUERY_FAIL",StringComparison.Ordinal)>=0||fault.IndexOf("CONFIRM_FAIL",StringComparison.Ordinal)>=0);uint observed;int observedError;if(TryActive(job,injectQuery,out observed,out observedError)){activeKnown=true;active=observed;}else{activeKnown=false;queryError=observedError;authorityFailure=true;}
            bool tasksComplete=(inputTask==null||inputTask.IsCompleted)&&(stdoutTask==null||stdoutTask.IsCompleted)&&(stderrTask==null||stderrTask.IsCompleted);
            if(parentSignaled&&activeKnown&&active==0&&tasksComplete)break;Thread.Sleep(2);
        }
        if(!parentSignaled&&clock.ElapsedMilliseconds<deadline)parentSignaled=WaitForSingleObject(pi.hProcess,0)==WAIT_OBJECT_0;
        if(clock.ElapsedMilliseconds<deadline){uint finalActive;int finalQueryError;bool injectFinalQuery=fault!=null&&(fault.IndexOf("FINAL_QUERY_FAIL",StringComparison.Ordinal)>=0||fault.IndexOf("CONFIRM_FAIL",StringComparison.Ordinal)>=0);if(TryActive(job,injectFinalQuery,out finalActive,out finalQueryError)){activeKnown=true;active=finalActive;}else{activeKnown=false;queryError=finalQueryError;authorityFailure=true;}}
        try{if(stdout!=null&&(stdoutTask==null||!stdoutTask.IsCompleted))stdout.Dispose();}catch{}try{if(stderr!=null&&(stderrTask==null||!stderrTask.IsCompleted))stderr.Dispose();}catch{}
        bool inputComplete=inputTask==null||inputTask.IsCompleted,stdoutComplete=stdoutTask==null||stdoutTask.IsCompleted,stderrComplete=stderrTask==null||stderrTask.IsCompleted;
        bool confirmed=parentSignaled&&activeKnown&&active==0&&inputComplete&&stdoutComplete&&stderrComplete&&!authorityFailure;
        bool treeConfirmed=parentSignaled&&activeKnown&&active==0;bool survivor=!treeConfirmed;
        string detail=primaryDetail+";parent_pid="+pi.dwProcessId+";thread_id="+pi.dwThreadId+";job_at_creation=True;job_contained=True;resumed="+resumed+";child_executed="+resumed+";job_terminated="+jobTerminated+";job_win32="+jobError.ToString(CultureInfo.InvariantCulture)+";process_terminated="+processTerminated+";process_win32="+processError.ToString(CultureInfo.InvariantCulture)+";parent_wait_win32="+waitError.ToString(CultureInfo.InvariantCulture)+";parent_signaled="+parentSignaled+";job_query_ok="+activeKnown+";job_query_win32="+queryError.ToString(CultureInfo.InvariantCulture)+";active="+(activeKnown?active.ToString(CultureInfo.InvariantCulture):"<unknown>")+";tree_terminated="+treeConfirmed+";stdout_complete="+stdoutComplete+";stderr_complete="+stderrComplete+";input_complete="+inputComplete+";elapsed_ms="+clock.ElapsedMilliseconds.ToString(CultureInfo.InvariantCulture)+";cleanup_ms="+cleanup.ElapsedMilliseconds.ToString(CultureInfo.InvariantCulture)+";survivor="+survivor;
        if(!confirmed)Fail("GIT_BATCH_PROCESS_TREE_TERMINATION_FAILED",detail);Fail(primaryCode,detail);
    }
    ...
    public static R7GitProcessResult[] RunConcurrentLaunchTest(string executable,string slowArguments,string fastArguments,string workingDirectory,int totalDeadlineMilliseconds,int cleanupReserveMilliseconds,int stderrLimit){
        lock(ConcurrentTestLock){using(Barrier barrier=new Barrier(2)){ConcurrentLaunchBarrier=barrier;try{R7GitExpectedObject[] none=new R7GitExpectedObject[0];Task<R7GitProcessResult> slow=Task.Factory.StartNew<R7GitProcessResult>(delegate{return RunCore(executable,slowArguments,workingDirectory,new byte[0],none,totalDeadlineMilliseconds,cleanupReserveMilliseconds,cleanupReserveMilliseconds,stderrLimit,"CONCURRENT_GATE");},CancellationToken.None,TaskCreationOptions.LongRunning,TaskScheduler.Default);Task<R7GitProcessResult> fast=Task.Factory.StartNew<R7GitProcessResult>(delegate{return RunCore(executable,fastArguments,workingDirectory,new byte[0],none,totalDeadlineMilliseconds,cleanupReserveMilliseconds,cleanupReserveMilliseconds,stderrLimit,"CONCURRENT_GATE");},CancellationToken.None,TaskCreationOptions.LongRunning,TaskScheduler.Default);Task.WaitAll(slow,fast);return new R7GitProcessResult[]{slow.Result,fast.Result};}finally{ConcurrentLaunchBarrier=null;}}}
    }
    static R7GitProcessResult RunCore(string executable,string arguments,string workingDirectory,byte[] standardInput,R7GitExpectedObject[] expected,int totalDeadlineMilliseconds,int cleanupReserveMilliseconds,int terminationConfirmationMilliseconds,int stderrLimit,string testFault){
...
function InvokeGitP03AFixture([string]$Mutation) {
    ...
    $invokeFault={param([string]$SelectedFault) InvokeBoundedGitBatchProcessLaunchTest $exe $arguments ([IO.Path]::GetTempPath()) ([byte[]]@()) @() $SelectedFault $deadline $reserve 65536}
    try{
        switch($Mutation){
            ...
            'P03A_JOB_AT_CREATION_REJECTION'{$fault='JOB_AT_CREATION_REJECT'}
            'P03A_NO_CHILD_ON_CREATION_REJECTION'{$fault='JOB_AT_CREATION_REJECT_NO_CHILD'}
            ...
            'P03A_RESUME_JOB_CLEANUP'{$fault='RESUME_ACTUAL_FAIL'}
            ...
            'P03A_CLEANUP_DEADLINE_EXHAUSTION'{$fault='RESUME_ACTUAL_FAIL_CLEANUP_DEADLINE'}
            ...
            'P03A_TASKS_TERMINAL'{$fault='RESUME_ACTUAL_FAIL_TASK_TERMINAL'}
            'P03A_ORIGINAL_DEADLINE_BOUND'{$fault='RESUME_ACTUAL_FAIL_PARENT_WAIT_FAIL_DEADLINE'}
            'P03A_NO_CHILD_REPEATED_CREATE_FAILURE'{for($i=0;$i-lt50;$i++){try{&$invokeFault 'JOB_AT_CREATION_REJECT';throw 'P03A_UNEXPECTED_PASS'}catch{if($_.Exception.Message-notmatch'^GIT_BATCH_JOB_AT_CREATION_FAILED\|'){throw}}};throw ([IO.InvalidDataException]::new('P03A_REPEATED_CREATE_FAILURE_CONFIRMED|count=50;child_created=False;survivor=False'))}
            'P03A_REPEATED_JOB_AT_CREATION_FAILURES'{for($i=0;$i-lt50;$i++){try{&$invokeFault 'JOB_AT_CREATION_REJECT_REPEAT';throw 'P03A_UNEXPECTED_PASS'}catch{if($_.Exception.Message-notmatch'^GIT_BATCH_JOB_AT_CREATION_FAILED\|'){throw}}};throw ([IO.InvalidDataException]::new('P03A_REPEATED_CREATE_FAILURE_CONFIRMED|count=50;child_created=False;survivor=False'))}
            'P03A_ATTRIBUTE_ALLOCATION_CLEANUP'{foreach($selected in @('JOB_LIST_INIT_FAIL','JOB_LIST_UPDATE_FAIL')){for($i=0;$i-lt25;$i++){try{&$invokeFault $selected;throw 'P03A_UNEXPECTED_PASS'}catch{if($_.Exception.Message-notmatch'^GIT_BATCH_(JOB_LIST_ATTRIBUTE_FAILED|CREATE_PROCESS_FAILED)\|'){throw}}}};throw ([IO.InvalidDataException]::new('P03A_ATTRIBUTE_CLEANUP_CONFIRMED|count=50;child_created=False'))}
            'P03A_REPEATED_RESUME_FAILURE'{for($i=0;$i-lt50;$i++){try{&$invokeFault 'RESUME_ACTUAL_FAIL';throw 'P03A_UNEXPECTED_PASS'}catch{if($_.Exception.Message-notmatch'^GIT_BATCH_PROCESS_RESUME_FAILED\|'-or$_.Exception.Message-notmatch'survivor=False'){throw}}};throw ([IO.InvalidDataException]::new('P03A_REPEATED_RESUME_FAILURE_CONFIRMED|count=50;child_executed=False;survivor=False'))}
            'P03A_AUTHENTIC_CONTAINED_LAUNCH'{$clean='-NoProfile -NonInteractive -EncodedCommand '+(EncodeLifecycleCommand 'exit 0');$result=InvokeBoundedGitBatchProcess $exe $clean ([IO.Path]::GetTempPath()) ([byte[]]@()) @() $deadline $reserve 65536;if(-not$result.JobAtCreationContained-or-not$result.ProcessTreeTerminated-or-not$result.ProcessExited){throw 'P03A_POSITIVE_INCOMPLETE'};throw ([IO.InvalidDataException]::new('P03A_AUTHENTIC_CONTAINED_LAUNCH_CONFIRMED|parent_pid='+$result.ParentPid+';thread_id='+$result.ThreadId+';job_at_creation=True;tree_terminated=True'))}
            default{throw 'Unknown P-03A fixture mutation: '+$Mutation}
        }
        try{&$invokeFault $fault;throw 'P03A_UNEXPECTED_PASS'}catch{if($_.Exception.Message-ceq'P03A_UNEXPECTED_PASS'){throw};if(Test-Path -LiteralPath $marker){throw ([IO.InvalidDataException]::new('P03A_CHILD_EXECUTION_DETECTED|mutation='+$Mutation))};throw ([IO.InvalidDataException]::new($_.Exception.Message+';marker_absent=True'))}
