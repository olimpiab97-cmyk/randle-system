...
public static class R7GitBatchProcessRunner {
    ...
    sealed class CleanupSnapshot {
        public readonly bool ParentSignaled,ZeroProcessAuthority,Unknown;public readonly string JobQueryStatus;public readonly uint ActiveProcesses;public readonly TaskObservation Input,Stdout,Stderr;public readonly long ElapsedMilliseconds,RemainingMilliseconds;
        public CleanupSnapshot(bool parent,string query,uint active,TaskObservation input,TaskObservation stdout,TaskObservation stderr,long elapsed,long remaining){ParentSignaled=parent;JobQueryStatus=query;ActiveProcesses=active;Input=input;Stdout=stdout;Stderr=stderr;ElapsedMilliseconds=elapsed;RemainingMilliseconds=remaining;ZeroProcessAuthority=parent&&query=="Succeeded"&&active==0;Unknown=query!="Succeeded"||!parent||!input.IsTerminal||!stdout.IsTerminal||!stderr.IsTerminal;}
    ...
    static CleanupSnapshot CaptureSnapshot(IntPtr process,IntPtr job,Stopwatch clock,int deadline,Task inputTask,Task stdoutTask,Task stderrTask,string phase,long phaseStart,bool injectWaitFailure,bool injectQueryFailure,out int waitError,out int queryError){
        ...
        return new CleanupSnapshot(parent,query,active,ObserveTask(inputTask,clock,phase),ObserveTask(stdoutTask,clock,phase),ObserveTask(stderrTask,clock,phase),Math.Max(0,clock.ElapsedMilliseconds-phaseStart),Math.Max(0,deadline-clock.ElapsedMilliseconds));
    }
    static CleanupSnapshot UnmeasuredSnapshot(Stopwatch clock,int deadline,Task inputTask,Task stdoutTask,Task stderrTask,string phase,long phaseStart){return new CleanupSnapshot(false,"NotAttempted",UInt32.MaxValue,ObserveTask(inputTask,clock,phase),ObserveTask(stdoutTask,clock,phase),ObserveTask(stderrTask,clock,phase),Math.Max(0,clock.ElapsedMilliseconds-phaseStart),Math.Max(0,deadline-clock.ElapsedMilliseconds));}
    ...
    static void CleanupCreatedProcessAndFail(string primaryCode,string primaryDetail,string fault,ref PROCESS_INFORMATION pi,IntPtr job,Stopwatch clock,int deadline,int confirmationMilliseconds,
        ...
        Stopwatch cleanup=Stopwatch.StartNew();long primaryStart=clock.ElapsedMilliseconds;long primaryEnd=Math.Min((long)deadline,primaryStart+(long)confirmationMilliseconds);bool jobTerminated=false,processTerminated=false;int jobError=0,processError=0,primaryWaitError=0,primaryQueryError=0,finalWaitError=0,finalQueryError=0;
        ...
        bool injectParentWait=fault!=null&&fault.IndexOf("PARENT_WAIT_FAIL",StringComparison.Ordinal)>=0;bool injectFinalQuery=fault!=null&&(fault.IndexOf("FINAL_QUERY_FAIL",StringComparison.Ordinal)>=0||fault.IndexOf("CONFIRM_FAIL",StringComparison.Ordinal)>=0);CleanupSnapshot primary=null,final=null;
        while(clock.ElapsedMilliseconds<primaryEnd){int waitError,queryError;primary=CaptureSnapshot(pi.hProcess,job,clock,deadline,inputTask,stdoutTask,stderrTask,"Primary",primaryStart,injectParentWait,injectFinalQuery,out waitError,out queryError);if(primaryWaitError==0)primaryWaitError=waitError;if(primaryQueryError==0)primaryQueryError=queryError;if(primary.CleanupTerminal&&waitError==0&&queryError==0)break;Thread.Sleep(2);}
        if(primary==null)primary=UnmeasuredSnapshot(clock,deadline,inputTask,stdoutTask,stderrTask,"Primary",primaryStart);else if(!(primary.CleanupTerminal&&primaryWaitError==0&&primaryQueryError==0)){int boundaryWait,boundaryQuery;primary=CaptureSnapshot(pi.hProcess,job,clock,deadline,inputTask,stdoutTask,stderrTask,"Primary",primaryStart,injectParentWait,injectFinalQuery,out boundaryWait,out boundaryQuery);if(primaryWaitError==0)primaryWaitError=boundaryWait;if(primaryQueryError==0)primaryQueryError=boundaryQuery;}bool primaryConfirmed=primary.CleanupTerminal&&primaryWaitError==0&&primaryQueryError==0;long finalStart=clock.ElapsedMilliseconds;
        while(clock.ElapsedMilliseconds<deadline){int waitError,queryError;final=CaptureSnapshot(pi.hProcess,job,clock,deadline,inputTask,stdoutTask,stderrTask,"Final",finalStart,false,injectFinalQuery,out waitError,out queryError);if(finalWaitError==0)finalWaitError=waitError;if(finalQueryError==0)finalQueryError=queryError;if(final.CleanupTerminal&&waitError==0&&queryError==0)break;Thread.Sleep(2);}
        if(final==null)final=UnmeasuredSnapshot(clock,deadline,inputTask,stdoutTask,stderrTask,"Final",finalStart);
        try{if(stdout!=null&&!final.Stdout.IsTerminal)stdout.Dispose();}catch{}try{if(stderr!=null&&!final.Stderr.IsTerminal)stderr.Dispose();}catch{}
        bool finalConfirmed=final.CleanupTerminal&&finalWaitError==0&&finalQueryError==0;bool treeConfirmed=final.ZeroProcessAuthority;bool survivor=!treeConfirmed;
        string detail=primaryDetail+";parent_pid="+pi.dwProcessId+";thread_id="+pi.dwThreadId+";job_at_creation=True;job_contained=True;resumed="+resumed+";child_executed="+resumed+";job_terminated="+jobTerminated+";job_win32="+jobError.ToString(CultureInfo.InvariantCulture)+";process_terminated="+processTerminated+";process_win32="+processError.ToString(CultureInfo.InvariantCulture)+";primary_wait_win32="+primaryWaitError.ToString(CultureInfo.InvariantCulture)+";primary_query_win32="+primaryQueryError.ToString(CultureInfo.InvariantCulture)+";final_wait_win32="+finalWaitError.ToString(CultureInfo.InvariantCulture)+";final_query_win32="+finalQueryError.ToString(CultureInfo.InvariantCulture)+";parent_wait_win32="+primaryWaitError.ToString(CultureInfo.InvariantCulture)+";parent_signaled="+final.ParentSignaled+";job_query_ok="+(final.JobQueryStatus=="Succeeded")+";job_query_win32="+finalQueryError.ToString(CultureInfo.InvariantCulture)+";active="+(final.JobQueryStatus=="Succeeded"?final.ActiveProcesses.ToString(CultureInfo.InvariantCulture):"<unknown>")+";tree_terminated="+treeConfirmed+";stdout_complete="+final.Stdout.IsSuccessful+";stderr_complete="+final.Stderr.IsSuccessful+";input_complete="+final.Input.IsSuccessful+";elapsed_ms="+clock.ElapsedMilliseconds.ToString(CultureInfo.InvariantCulture)+";cleanup_ms="+cleanup.ElapsedMilliseconds.ToString(CultureInfo.InvariantCulture)+";survivor="+survivor+";"+SnapshotDetail("primary",primary)+";"+SnapshotDetail("final",final);
...
function InvokeGitP03AFixture([string]$Mutation) {
    ...
    try{
        ...
        switch($Mutation){
            ...
            'P03A_CLEANUP_DEADLINE_EXHAUSTION'{$fault='RESUME_ACTUAL_FAIL_FINAL_QUERY_FAIL_TOTAL_DEADLINE'}
            ...
            'P03A_ORIGINAL_DEADLINE_BOUND'{$fault='RESUME_ACTUAL_FAIL_PARENT_WAIT_FAIL_PRIMARY_WINDOW';$confirmation=1}
