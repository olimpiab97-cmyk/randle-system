...
public static class R7GitBatchProcessRunner {
    ...
    static R7GitProcessResult RunCore(string executable,string arguments,string workingDirectory,byte[] standardInput,R7GitExpectedObject[] expected,int totalDeadlineMilliseconds,int cleanupReserveMilliseconds,int terminationConfirmationMilliseconds,int stderrLimit,string testFault,string readinessPath,int readinessCount,string readinessNonce,string readinessCase){
        ...
        try{
            ...
            Stream outLocal=stdout,errLocal=stderr,inLocal=stdin;stdoutTask=Task.Factory.StartNew<R7GitAuthenticatedObject[]>(delegate{stdoutWorker.Attach();try{if(readinessGated||n149ActivityGate){byte[] proofBytes=DrainReadinessProof(outLocal,4096,"stdout",readinessProof);Interlocked.Exchange(ref observedStdout,proofBytes.LongLength);return new R7GitAuthenticatedObject[0];}return Parse(outLocal,expected,aggregate,delegate(long value){Interlocked.Exchange(ref observedStdout,value);});}finally{stdoutWorker.Detach();}},CancellationToken.None,TaskCreationOptions.LongRunning,TaskScheduler.Default);stderrTask=Task.Factory.StartNew<StderrState>(delegate{stderrWorker.Attach();try{if(readinessGated||n149ActivityGate){byte[] proofBytes=DrainReadinessProof(errLocal,Math.Min(stderrLimit,4096),"stderr",readinessProof);return new StderrState{Bytes=proofBytes,Total=proofBytes.LongLength};}return DrainStderr(errLocal,stderrLimit);}finally{stderrWorker.Detach();}},CancellationToken.None,TaskCreationOptions.LongRunning,TaskScheduler.Default);inputCancellation=new CancellationTokenSource();if(testFault=="POST_TASK_INPUT_CANCELED")inputCancellation.Cancel();CancellationToken inputToken=inputCancellation.Token;inputTask=Task.Factory.StartNew(delegate{inputWorker.Attach();try{if(testFault=="POST_TASK_INPUT_FAULT")throw new InvalidDataException("GIT_BATCH_INPUT_TASK_FAULT|stage=input-task");if(standardInput!=null&&standardInput.Length!=0)inLocal.Write(standardInput,0,standardInput.Length);inLocal.Flush();inLocal.Dispose();}finally{inputWorker.Detach();}},inputToken,TaskCreationOptions.LongRunning,TaskScheduler.Default);if(testFault=="POST_TASK_INPUT_CANCELED")failure=new InvalidDataException("GIT_BATCH_INPUT_TASK_CANCELED|stage=input-task");if(testFault!=null&&testFault.StartsWith("POST_TASK_FORCED_CLEANUP",StringComparison.Ordinal))failure=new InvalidDataException("GIT_BATCH_POST_RESUME_CLEANUP_FAILED|stage=post-task-distinct;case_selector="+testFault);if(!readinessGated&&!n149ActivityGate&&testFault!=null&&testFault.IndexOf("PRIMARY_LATE_PROCESS_TRANSITION",StringComparison.Ordinal)>=0)failure=new InvalidDataException("GIT_BATCH_PROCESS_TIMEOUT|stage=late-primary-transition;case_selector="+testFault);
...
function InvokeGitXAuthorityFixture([string]$Mutation) {
    InitializeGitBatchProcessRuntime;$exe=(Get-Process -Id $PID).Path;$deadline=2500;$reserve=700;$confirmation=100;$code='Start-Sleep -Seconds 20';$fault='';$expected=@()
    ...
    elseif($Mutation-like'X03_*'){
        ...
        switch($Mutation){
            'X03_STDOUT_RUNNING_UNTIL_CLOSE'{$code='[Console]::Error.Close();Start-Sleep -Seconds 20'}
    ...
    $arguments='-NoProfile -NonInteractive -EncodedCommand '+(EncodeLifecycleCommand $code);$clock=[Diagnostics.Stopwatch]::StartNew()
    try{InvokeBoundedGitBatchProcessLaunchTest $exe $arguments ([IO.Path]::GetTempPath()) ([byte[]]@()) $expected $fault $deadline $reserve 65536 $confirmation|Out-Null;throw 'X_AUTHORITY_UNEXPECTED_PASS'}catch{
