...
public static class R7GitBatchProcessRunner {
    ...
    static IntPtr BuildAttributeList(IntPtr stdinRead,IntPtr stdoutWrite,IntPtr stderrWrite,string fault,out IntPtr handleValues){
        ...
        UIntPtr bytes=UIntPtr.Zero;bool sized=InitializeProcThreadAttributeList(IntPtr.Zero,1,0,ref bytes);int sizeError=Marshal.GetLastWin32Error();if(sized||bytes==UIntPtr.Zero||sizeError!=122)Fail("GIT_BATCH_ATTRIBUTE_LIST_INITIALIZATION_FAILED","stage=size;win32="+sizeError.ToString(CultureInfo.InvariantCulture));
        if(bytes.ToUInt64()>Int32.MaxValue)Fail("GIT_BATCH_ATTRIBUTE_LIST_INITIALIZATION_FAILED","stage=size;reason=overflow");IntPtr list=Marshal.AllocHGlobal((int)bytes.ToUInt64());handleValues=IntPtr.Zero;
        bool initialized=false;try{if(fault=="ATTRIBUTE_INIT_FAIL"||!InitializeProcThreadAttributeList(list,1,0,ref bytes)){int error=fault=="ATTRIBUTE_INIT_FAIL"?5:Marshal.GetLastWin32Error();Fail("GIT_BATCH_ATTRIBUTE_LIST_INITIALIZATION_FAILED","stage=initialize;win32="+error.ToString(CultureInfo.InvariantCulture));}initialized=true;handleValues=Marshal.AllocHGlobal(IntPtr.Size*3);Marshal.WriteIntPtr(handleValues,0,stdinRead);Marshal.WriteIntPtr(handleValues,IntPtr.Size,stdoutWrite);Marshal.WriteIntPtr(handleValues,IntPtr.Size*2,stderrWrite);if(fault=="HANDLE_UPDATE_FAIL"||!UpdateProcThreadAttribute(list,0,PROC_THREAD_ATTRIBUTE_HANDLE_LIST,handleValues,new UIntPtr((uint)(IntPtr.Size*3)),IntPtr.Zero,IntPtr.Zero)){int error=fault=="HANDLE_UPDATE_FAIL"?5:Marshal.GetLastWin32Error();Fail("GIT_BATCH_HANDLE_LIST_UPDATE_FAILED","win32="+error.ToString(CultureInfo.InvariantCulture));}return list;}catch{if(handleValues!=IntPtr.Zero){Marshal.FreeHGlobal(handleValues);handleValues=IntPtr.Zero;}if(initialized)DeleteProcThreadAttributeList(list);Marshal.FreeHGlobal(list);throw;}
    }
    ...
    static R7GitProcessResult RunCore(string executable,string arguments,string workingDirectory,byte[] standardInput,R7GitExpectedObject[] expected,int totalDeadlineMilliseconds,int cleanupReserveMilliseconds,int terminationConfirmationMilliseconds,int stderrLimit,string testFault){
        ...
        Stopwatch clock=Stopwatch.StartNew();long executionCutoff=totalDeadlineMilliseconds-cleanupReserveMilliseconds;IntPtr job=IntPtr.Zero,stdoutRead=IntPtr.Zero,stdoutWrite=IntPtr.Zero,stderrRead=IntPtr.Zero,stderrWrite=IntPtr.Zero,stdinRead=IntPtr.Zero,stdinWrite=IntPtr.Zero,attributes=IntPtr.Zero,attributeHandles=IntPtr.Zero,environment=IntPtr.Zero;PROCESS_INFORMATION pi=new PROCESS_INFORMATION();FileStream stdout=null,stderr=null,stdin=null;Task<R7GitAuthenticatedObject[]> stdoutTask=null;Task<StderrState> stderrTask=null;Task inputTask=null;Exception failure=null;long cleanupMs=0;bool processExited=false,treeTerminated=false;string environmentNames="";
        try{
            ...
            environment=BuildEnvironment(testFault,out environmentNames);if(environmentNames!="COMSPEC,SystemRoot,TEMP,TMP,WINDIR")Fail("GIT_BATCH_ENVIRONMENT_INVALID","reason=contract-order;observed="+environmentNames);attributes=BuildAttributeList(stdinRead,stdoutWrite,stderrWrite,testFault,out attributeHandles);
            job=CreateJobObject(IntPtr.Zero,null);if(job==IntPtr.Zero)Native("CreateJobObject");ConfigureJob(job);
            STARTUPINFOEX si=new STARTUPINFOEX();si.StartupInfo.cb=Marshal.SizeOf(typeof(STARTUPINFOEX));si.StartupInfo.dwFlags=STARTF_USESTDHANDLES;si.StartupInfo.hStdInput=stdinRead;si.StartupInfo.hStdOutput=stdoutWrite;si.StartupInfo.hStdError=stderrWrite;si.lpAttributeList=attributes;StringBuilder command=new StringBuilder(Quote(executable)+(String.IsNullOrEmpty(arguments)?"":" "+arguments));if(!CreateProcessW(executable,command,IntPtr.Zero,IntPtr.Zero,true,CREATE_SUSPENDED|CREATE_NO_WINDOW|CREATE_UNICODE_ENVIRONMENT|EXTENDED_STARTUPINFO_PRESENT,environment,workingDirectory,ref si,out pi)){int error=Marshal.GetLastWin32Error();Fail("GIT_BATCH_CREATE_PROCESS_FAILED","win32="+error.ToString(CultureInfo.InvariantCulture));}
            int assignError=0;bool assignOk=testFault=="JOB_ASSIGN_FAIL"||testFault=="JOB_ASSIGN_TERMINATE_FAIL"||testFault=="JOB_ASSIGN_CONFIRM_FAIL"?false:AssignProcessToJobObject(job,pi.hProcess);if(!assignOk){assignError=testFault!=null?5:Marshal.GetLastWin32Error();bool firstTermination=testFault=="JOB_ASSIGN_TERMINATE_FAIL"?false:TerminateProcess(pi.hProcess,221);int terminateError=firstTermination?0:(testFault=="JOB_ASSIGN_TERMINATE_FAIL"?5:Marshal.GetLastWin32Error());bool fallbackTermination=true;if(!firstTermination)fallbackTermination=TerminateProcess(pi.hProcess,222);bool signaled=WaitSignaled(pi.hProcess,clock,totalDeadlineMilliseconds);uint active=Active(job);string detail="win32="+assignError.ToString(CultureInfo.InvariantCulture)+";terminate_win32="+terminateError.ToString(CultureInfo.InvariantCulture)+";fallback_terminated="+fallbackTermination+";"+LaunchDetail(pi,clock,false,false,signaled,active);if(testFault=="JOB_ASSIGN_CONFIRM_FAIL"||!firstTermination||!fallbackTermination||!signaled)Fail("GIT_BATCH_UNCONTAINED_CHILD_TERMINATION_FAILED",detail);Fail("GIT_BATCH_JOB_ASSIGNMENT_FAILED",detail);}
            uint assignedActive=0;try{assignedActive=Active(job);}catch(Exception confirmationFailure){bool processKilled=TerminateProcess(pi.hProcess,223);bool signaled=WaitSignaled(pi.hProcess,clock,totalDeadlineMilliseconds);Fail("GIT_BATCH_PROCESS_TREE_TERMINATION_FAILED","reason=assignment-confirmation-query-failed;failure_type="+confirmationFailure.GetType().Name+";process_terminated="+processKilled+";"+LaunchDetail(pi,clock,true,false,signaled,0));}
            if(assignedActive!=1){bool killed=TerminateJobObject(job,223);bool processFallback=WaitForSingleObject(pi.hProcess,0)==WAIT_OBJECT_0||TerminateProcess(pi.hProcess,224);bool signaled=WaitSignaled(pi.hProcess,clock,totalDeadlineMilliseconds);uint remaining=Active(job);Fail("GIT_BATCH_PROCESS_TREE_TERMINATION_FAILED","reason=assignment-not-confirmed;terminated="+killed+";process_fallback="+processFallback+";"+LaunchDetail(pi,clock,true,false,signaled,remaining));}
            uint resume=testFault=="RESUME_FAIL"||testFault=="RESUME_TERMINATE_JOB_FAIL"||testFault=="RESUME_CONFIRM_FAIL"?UInt32.MaxValue:ResumeThread(pi.hThread);if(resume!=1){int resumeError=testFault!=null?5:Marshal.GetLastWin32Error();bool firstTermination=testFault=="RESUME_TERMINATE_JOB_FAIL"?false:TerminateJobObject(job,224);int terminateError=firstTermination?0:(testFault=="RESUME_TERMINATE_JOB_FAIL"?5:Marshal.GetLastWin32Error());bool fallbackTermination=true;if(!firstTermination)fallbackTermination=TerminateJobObject(job,225);bool processFallback=true;if(Active(job)!=0)processFallback=TerminateProcess(pi.hProcess,226);bool signaled=WaitSignaled(pi.hProcess,clock,totalDeadlineMilliseconds);uint active=Active(job);string detail="win32="+resumeError.ToString(CultureInfo.InvariantCulture)+";terminate_win32="+terminateError.ToString(CultureInfo.InvariantCulture)+";fallback_terminated="+fallbackTermination+";process_fallback="+processFallback+";"+LaunchDetail(pi,clock,true,false,signaled,active);if(testFault=="RESUME_CONFIRM_FAIL"||!firstTermination||!fallbackTermination||!processFallback||!signaled||active!=0)Fail("GIT_BATCH_PROCESS_TREE_TERMINATION_FAILED",detail);Fail("GIT_BATCH_PROCESS_RESUME_FAILED",detail);}
...
function InvokeNegativeCases([object]$Receipt,[object]$Determinism,[object]$NegativeRegistry) {
    ...
    foreach($case in @($NegativeRegistry.cases)){
        $mutated=Clone $Receipt;$detMutated=Clone $Determinism;$overrides=@{};$provenance=@{};$role=$mutated.roles[0];$directParserMutation=$null;$directLifecycleMutation=$null;$directLaunchMutation=$null
        ...
        try{if($null-ne$directParserMutation){[void](InvokeGitBatchParserFixture $directParserMutation)}elseif($null-ne$directLifecycleMutation){[void](InvokeGitBatchLifecycleFixture $directLifecycleMutation)}elseif($null-ne$directLaunchMutation){[void](InvokeGitLaunchAuthorityFixture $directLaunchMutation)}else{ValidateModel $mutated $detMutated $overrides $provenance};throw 'NEGATIVE_CASE_UNEXPECTED_PASS'}catch{if($_.Exception.Message-ceq'NEGATIVE_CASE_UNEXPECTED_PASS'){throw};$parts=$_.Exception.Message-split'\|',2;$observed=$parts[0];if($parts.Count-gt1){$observedDetail=$parts[1]}}
