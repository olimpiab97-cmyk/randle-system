...
Three Unit 2 bootstrap invocations are preserved as nonauthority failure evidence. The first stopped when `sc.exe create` rejected its argument framing with exit 1639 and created only twelve empty dedicated directories. The second created the stopped, restricted service entry but stopped when the CNG software KSP rejected legacy `KeySpec Signature`; it created no certificate or key. The third used CNG `KeySpec None`, created one nonexportable LocalMachine key and one public certificate, and applied the dedicated root/key ACLs, but stopped when a redundant certificate-file ACL rewrite returned access denied after leaving that public file with an empty protected DACL. It installed no executable, policy, ledger, or authorization and never started the service. The corrective bootstrap requires all three exact failure-evidence hashes, validates the exact stopped service/certificate/key state without reading or exporting private bytes, uses the content-bound absolute `takeown.exe` only on that public certificate file, restores a closed public-file ACL, reduces Administrators on the key file to metadata-only rights, and binds every interruption into the later authority payload.
...
`external_utility_registry.json` content-binds the absolute Git, PowerShell, compiler, IL, framework/reference, service-control, ACL, one-time certificate-owner recovery, filesystem, job, management, utility-module, and PKI inputs. Unit 2 permits `takeown.exe` only for the exact preserved public certificate file and permits the other content-bound SCM, ACL, PKI, and PowerShell inputs only against the new upgrade boundary; terminal-service mutation remains forbidden. `R7MeasuredUtility.cs` defines held-handle identity checks. Static measurements do not prove installed runtime dependency closure.
