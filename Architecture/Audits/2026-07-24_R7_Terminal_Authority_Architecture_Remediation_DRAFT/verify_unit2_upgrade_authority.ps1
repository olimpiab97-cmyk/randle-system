...
$adminRights=[int64]$adminRule[0].FileSystemRights;$allowedAdminRights=[int64]([Security.AccessControl.FileSystemRights]::ReadAttributes -bor [Security.AccessControl.FileSystemRights]::ReadPermissions);if(($adminRights -band (-bnot $allowedAdminRights)) -ne 0 -or ($adminRights -band $allowedAdminRights) -ne $allowedAdminRights){throw 'Upgrade private-key ACL does not grant exactly the required Administrator metadata-read rights'}
