...
The Entry Agent Session Commit Writer is the sole canonical writer and transaction owner for the Entry Session Aggregate, canonical session-lock fact, active pointer, atomic candidate activation, and atomic prior-session retirement. Session-lock policy supplies eligibility/decision input only and SHALL NOT write the aggregate, lock fact, active pointer, transaction, activation, or retirement. TradingView owns the transmitted source payload. The local Health Durable Writer owns durable health commit/cursor mutation.
...
Retain ADR-014 under approved ADRs as the existing governing Entry-session decision. Add ADR-015, ADR-016, and active supporting draft specifications under `Current Draft Items`, explicitly labeled noncanonical/not approved. Mark `docs/architecture/listener_supervision_and_health_authority_DRAFT.md` as `WITHDRAWN — SUPERSEDED DRAFT`, noncanonical, historical evidence only, and remove it from active normative/implementation dependency lists. Do not list ADR-015/ADR-016 under approved ADRs until separate approval and canonical incorporation. Add the Phase 3A remediation record, completed clause registry, conflict matrix, and package traceability record as nonauthoritative evidence indexes.
...
## 18. Phase 3A clause traceability and storage amendment

If separately approved and later authorized for incorporation:

1. `Architecture/Audits/2026-07-17_ADR015_016_Clause_Traceability_Registry_DRAFT.md` SHALL be incorporated as the clause-level forward/reverse traceability source for every mandatory ADR-015, ADR-016, Entry Session, Startup, and Diagnostic Purity obligation. Broad section mappings SHALL NOT substitute for its stable requirement IDs.
2. The physical runtime-authority store SHALL use Pattern A: one SQLite database with ownership-separated tables and exact logical writer authorization. The mechanical transaction coordinator SHALL NOT become a domain authority or writer. No duplicate authoritative identity copy or unenforceable cross-database foreign key is permitted.
