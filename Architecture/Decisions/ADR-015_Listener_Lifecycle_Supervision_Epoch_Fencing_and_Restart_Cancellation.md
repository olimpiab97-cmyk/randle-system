...
**Draft version:** Phase 3B remediation
...
The Listener Supervisor SHALL use the one physical Runtime Authority Control Database defined jointly with ADR-016 at the absolute path resolved once at startup from `%LOCALAPPDATA%\RandleRuntimeData\control\runtime_authority_v1.sqlite3`. The resolved path SHALL be recorded in startup evidence. The complete DDL-equivalent schema, writer registry, constraint set, typed transaction catalog, crash rules, and reconstruction order are normative in `docs/architecture/runtime_authority_store_schema_DRAFT.md` sections 2 through 10. This is Pattern A: one SQLite database contains ownership-separated supervisor, listener-epoch, listener-lifecycle, listener-incident, acknowledgement, bridge, health, and termination tables so every declared foreign key is physically enforceable. Shared physical storage SHALL NOT merge authority ownership.

The in-process `Runtime Authority Store Transaction Coordinator` SHALL own the only read-write SQLite connection and SHALL mechanically serialize approved typed transaction plans; it owns no lifecycle, health, policy, or evidence decision. The closed logical writer allowlist is the Runtime Authority Store Schema section 6: `Supervisor Generation Writer` writes `supervisor_generations`/`supervisor_leases`; `Listener Epoch Writer` writes `listener_epochs`; `Listener State Writer` writes `listener_current`/`listener_state_transitions`; `Listener Incident Writer` writes listener incident, outcome, fence, execution, rehydration, policy, and expectation tables; `Listener Acknowledgement Writer` writes `recovery_required_domains`/`domain_acknowledgements`; `Recovery Transaction Writer` alone writes `recovery_transactions`; `Bridge Generation Writer` writes `bridge_generations`; the ADR-016 Health Durable Writer writes bridge, health, producer, subscription, termination, projection-cursor, and validated external-reference tables; and `Store Incident Writer` alone writes `store_incidents`. SQLite authorizer rules and the immutable versioned `writer_registry` SHALL reject a transaction plan that targets a table outside its named writer. The coordinator SHALL NOT originate, reinterpret, combine, or repair a domain decision.
...
The closed listener transaction set is `TX-LSN-CANCEL`, `TX-LSN-FENCE`, `TX-LSN-EXECUTION-START`, `TX-LSN-REHYDRATION-START`, `TX-LSN-ACK`, `TX-LSN-COMPLETE`, `TX-LSN-FAIL`, `TX-LSN-RATE-EXHAUSTED`, and `TX-LSN-PLANNED-STOP`. Their exact preconditions, rows, writer sets, idempotency, expected versions, commit/readback, crash, replay, and rollback rules are Runtime Authority Store Schema sections 7.1 and 8. A lifecycle or incident transition outside that catalog is prohibited.
...
The full-listener lifecycle SHALL use only the following current-state path:
...
STOPPED
  ...
  -> STARTING
  -> REHYDRATING
  -> HEALTHY | LISTENER_FAILED | AMBIGUOUS_PROCESS_AUTHORITY | SUPERVISOR_STORE_FAILED
```
...
Within `TX-LSN-CANCEL`, after confirming the accepted recovery/liveness commit and before committing the incident outcome, the Listener Supervisor SHALL reevaluate the full current post-recovery evidence set. That typed transaction SHALL atomically write the incident transition, `RESTART_CANCELED` outcome, listener transition, `listener_current` version, incident version, evidence references, and transaction completion record. The resulting state SHALL be `HEALTHY` only when the canceled predicate and every other listener-level suspect condition are false; it SHALL be `SUSPECT` when any other listener-level condition remains unproven or degraded. No intermediate committed cancellation lacking this reevaluation is legal. This reevaluation SHALL NOT reuse the canceled incident as a restart action.
...
The Listener Supervisor is the sole transition authority for every full-listener and restart-incident row below. `Listener Supervisor Incident Writer` is the sole logical writer, and the Runtime Authority Store Transaction Coordinator is only the mechanical SQLite serializer.
...
| `STARTING` | Governed startup/restart execution exists and child identity is publication-fenced | execution/child token/intended epoch/start evidence | `REHYDRATING`, `LISTENER_FAILED`, `AMBIGUOUS_PROCESS_AUTHORITY`, `SUPERVISOR_STORE_FAILED` | Adopt the exact child/execution or remain blocked; never duplicate-start | Nonready | `RRV-LS-001`, `RRV-LS-002` |
| `REHYDRATING` | One granted new/current epoch awaits authoritative domain acknowledgements | recovery transaction and required acknowledgement identity set | `HEALTHY`, `LISTENER_FAILED`, `AMBIGUOUS_PROCESS_AUTHORITY`, `SUPERVISOR_STORE_FAILED` | Resume same recovery identity; Command Center cannot close it | Nonready | `RRV-LS-001`, `RRV-ATR-001`, `RRV-ST-001` |
...
| `SUSPECT` | One named current listener-health condition is degraded/unproven | exact predicate/evidence/stale boundary/current epoch | `HEALTHY`, `FENCED`, `STOPPING`, `LISTENER_FAILED`, `SUPERVISOR_STORE_FAILED` | Restore and reevaluate; a separate `RESTART_PENDING` incident may coexist | Blocks affected/all new entries | `RRV-LS-001`, `RRV-LS-003` |
| `FENCED` | Atomic `RESTART_PENDING -> RESTART_FENCED` transaction revokes current epoch publication | epoch fence, incident version, execution ID, fencing token | `STOPPING`, `LISTENER_FAILED`, `AMBIGUOUS_PROCESS_AUTHORITY`, `SUPERVISOR_STORE_FAILED` | Resume same execution; recovery data cannot unfence | Nonready; old epoch ineligible | `RRV-LS-001`, `RRV-LS-002` |
| `STOPPING` | Exact fenced stop command accepted for the one execution identity | process handle/command/ack/result evidence | `STARTING`, `LISTENER_FAILED`, `AMBIGUOUS_PROCESS_AUTHORITY`, `SUPERVISOR_STORE_FAILED` | Resolve exact process result; no second stop/start incident | Nonready | `RRV-LS-002` |
| `LISTENER_FAILED` | Current listener startup/stop/rehydration operation fails with authoritative evidence | failure stage, process/epoch/incident/execution identities | `STOPPED` only by governed containment; `STARTING` only through a new governed recovery incident | No automatic retry; retain evidence across restart | Nonready; entries blocked | `RRV-LS-002`, `RRV-LS-003` |
| `AMBIGUOUS_PROCESS_AUTHORITY` | Exact owned-process/adoption evidence cannot prove zero or one authoritative child | conflicting/unknown process identities and investigation evidence | Governed reconciliation result to `STOPPED`, `STARTING`, or `LISTENER_FAILED` | Prohibit another start until ambiguity is resolved durably | Nonready; entries blocked | `RRV-LS-002` |
| `SUPERVISOR_STORE_FAILED` | Runtime Authority Control Database integrity, writer routing, count, or current-authority invariant fails | store incident, failed checks, last verified cursor, recovery identity | Governed store recovery result only | No listener start/adopt/restart; restart does not clear | Nonready; entries blocked | `RRV-LS-002`, `RRV-FH-003` |
