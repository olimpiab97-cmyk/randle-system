...
The Listener Supervisor SHALL use the one physical Runtime Authority Control Database defined jointly with ADR-016 at the absolute path resolved once at startup from `%LOCALAPPDATA%\RandleRuntimeData\control\runtime_authority_v1.sqlite3`. The resolved path SHALL be recorded in startup evidence. This is Pattern A: one SQLite database contains ownership-separated supervisor, listener-epoch, bridge, health, and termination tables so every required foreign key is physically enforceable. Shared physical storage SHALL NOT merge authority ownership.

The in-process `Runtime Authority Store Transaction Coordinator` SHALL own the only read-write SQLite connection and SHALL mechanically serialize approved typed transaction plans; it owns no lifecycle, health, policy, or evidence decision. `Listener Supervisor Generation Writer` is the sole logical writer for `supervisor_generations` and `supervisor_leases`; `Listener Supervisor Epoch Writer` is the sole logical writer for `listener_epochs`; `Listener Supervisor Incident Writer` is the sole logical writer for `listener_restart_incidents`, `listener_restart_executions`, and authoritative-domain acknowledgement rows; and the ADR-016 Health Durable Writer is the sole logical writer for bridge, health, producer, and termination tables. SQLite authorizer rules and an immutable table-writer registry SHALL reject a transaction plan that targets a table outside its named writer. The coordinator SHALL NOT originate, reinterpret, combine, or repair a domain decision.

The database SHALL use WAL journaling, `synchronous=FULL`, `foreign_keys=ON`, explicit schema/version records, `BEGIN IMMEDIATE` transaction boundaries, compare-and-swap incident versions, and atomic supervisor-generation, lease, fence, execution, epoch, acknowledgement, and completion records. Transactions that span supervisor-owned tables remain authorized only by the Listener Supervisor transition authority. ADR-016 bridge/health transactions remain authorized only by the State Evaluator and written only through the Health Durable Writer. No local witness, cache, projection, or duplicate database may become a second identity authority.
...
Within the same serializable cancellation transaction, after confirming the accepted recovery/liveness commit and before committing the incident outcome, the Listener Supervisor SHALL reevaluate the full current post-recovery evidence set. The transaction SHALL atomically commit `RESTART_CANCELED` and the resulting current full-listener state. That state SHALL be `HEALTHY` only when the canceled predicate and every other listener-level suspect condition are false; it SHALL be `SUSPECT` when any other listener-level condition remains unproven or degraded. No intermediate committed cancellation lacking this reevaluation is legal. This reevaluation SHALL NOT reuse the canceled incident as a restart action.
...
The `Listener Supervision Policy Evaluator` is the sole evaluator and disposition authority. Its input identity SHALL contain the resolved artifact path, canonical bytes SHA-256, schema version, policy ID, topology version, deployment-authorization ID and bound digest, evaluator build, startup attempt, and evaluation sequence. The `Listener Supervisor Incident Writer` SHALL durably write the result to `policy_validation_results` in the Runtime Authority Control Database through the mechanical transaction coordinator. The record SHALL contain every failed field/rule, expected value/range, observed type/value or absence, duplicate/additional-key evidence, approval/deployment mismatch, exact reason code, evaluation time, and integrity identity.
