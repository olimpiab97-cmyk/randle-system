...
**Draft version:** Phase 3C1 normative remediation
...
The closed listener transaction set is `TX-LSN-CANCEL`, `TX-LSN-FENCE`, `TX-LSN-EXECUTION-START`, `TX-LSN-REHYDRATION-START`, `TX-LSN-ACK`, `TX-LSN-COMPLETE`, `TX-LSN-FAIL`, `TX-LSN-RATE-EXHAUSTED`, `TX-LSN-PLANNED-STOP`, and `TX-LSN-STOP-COMPLETE`. Their exact preconditions, rows, writer sets, idempotency, expected versions, commit/readback, crash, replay, rollback, and reconstruction rules are Runtime Authority Store Schema sections 9 and 11. A lifecycle or incident transition outside that catalog is prohibited.

Every exact writer-set declaration SHALL include `Listener Incident Writer` for `TX-LSN-EXECUTION-START`. `TX-LSN-CANCEL` uses Listener Incident + Listener State Writers; `TX-LSN-FENCE` uses Listener Incident + Listener State Writers; `TX-LSN-EXECUTION-START` uses Listener Incident Writer; `TX-LSN-REHYDRATION-START` uses Listener Incident + Recovery Transaction + Listener Acknowledgement + Listener State Writers; `TX-LSN-ACK` uses Listener Acknowledgement + Listener Incident Writers; `TX-LSN-COMPLETE`, `TX-LSN-FAIL`, and `TX-LSN-RATE-EXHAUSTED` use Listener Incident + applicable Recovery Transaction + Listener State Writers; and `TX-LSN-PLANNED-STOP`/`TX-LSN-STOP-COMPLETE` use Listener State Writer. No broader listener writer set is legal.
...
SUSPECT -> HEALTHY | FENCED | STOPPING | LISTENER_FAILED | SUPERVISOR_STORE_FAILED
