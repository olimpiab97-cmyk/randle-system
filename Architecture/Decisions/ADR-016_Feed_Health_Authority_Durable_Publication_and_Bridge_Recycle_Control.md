...
The record SHALL also carry `termination_observation_id`, supervisor generation, listener epoch, bridge generation, exact process identity, observation/event IDs, intent/incident/fence/execution IDs where present, raw callback/exit/exception references, observed/recorded times, schema version, and integrity checksum. `UNKNOWN` is independently valid for any field; uncertainty in one dimension SHALL NOT erase verified facts in another or authorize inference.
