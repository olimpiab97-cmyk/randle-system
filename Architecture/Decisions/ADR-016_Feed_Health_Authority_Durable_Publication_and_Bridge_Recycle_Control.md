...
**DRAFT 0.5 - PHASE 3C1-R1 TARGETED NORMATIVE REMEDIATION - NOT APPROVED**
...
3. require SQLite `>=3.43.1`, `application_id=0x52484C54`, `user_version=2`, exact schema identity/hash, bootstrap identity, writer-registry version/hash, store UUID, created time, 38 tables, 500 columns, 124 FK declarations/132 mappings, 13 partial unique indexes, and 14 triggers;
4. run `PRAGMA quick_check` and require the single result `ok`, then run `PRAGMA foreign_key_check` and require zero rows;
5. verify the contiguous transaction cursor/idempotency results, 60 active exclusive writer routes and succession ordering, unique event/producer sequences, every stored canonical-event checksum, supervisor-generation sequence, listener-epoch/bridge-generation ancestry, and incident/fence/execution state-machine legality;
...
The record SHALL also carry `termination_observation_id`, supervisor generation, listener epoch, bridge generation, exact process identity, observation sequence, request identity, operator command identity when applicable, OS/bridge/listener evidence identities when present, classification-decision identity, evaluator version, committing transaction, evidence-set hash, observed/recorded times, schema version, and integrity checksum. Exactly five `termination_result_evidence` rows bind one authenticated contributing evidence record to each independent dimension. `UNKNOWN` is independently valid for any field; uncertainty in one dimension SHALL NOT erase verified facts in another or authorize inference.

For every field, `NONE` is legal only when a complete trustworthy current-generation evidence set proves the absence of that dimension. A complete set requires an available and integrity-verified health/supervisor store through the observation cursor, authenticated current-generation producer and OS-adapter sequences with no gap across the observation interval, current epoch/generation identity, and successful queries of every authoritative intent, command, process-result, and provider-callback source applicable to that field. `NONE` means respectively: no authenticated initiator acted; no applicable request exists; no execution mechanism was attempted; no governed cause was observed; or no result has yet occurred for an otherwise open observation.

For every field, `UNKNOWN` is mandatory when any evidence needed to prove that dimension is missing, incomplete, sequence-gapped, conflicting, corrupt, stale, unavailable, unauthenticated, or belongs to another supervisor generation, listener epoch, bridge generation, process identity, or incident. A value proven for one field SHALL NOT be copied into another. `NONE` and `UNKNOWN` are not interchangeable and no implementation default may select between them.

The executable classification basis is exact: a `NONE` field requires its dimension link to be `COMPLETE_ABSENCE_PROOF`; an `UNKNOWN` field requires `UNCERTAINTY`; every other closed token requires `CONCRETE`. `BRIDGE_CRASH` additionally requires authenticated `PROCESS_EXCEPTION` or authoritative `OS_HANDLE` crash evidence. The result trigger rejects missing dimensions, arbitrary vocabulary, cross-generation evidence, and concrete crash without crash evidence.
...
Current semantic traceability is incomplete. `Architecture/Audits/2026-07-17_ADR015_016_Clause_Traceability_Registry_DRAFT.md` is historical rejected Phase 3B evidence, not a current forward/reverse mapping. Phase 3C2 will rebuild clause/scenario/assertion traceability only from independently accepted Phase 3C1-R1 hashes. The external recovery matrix remains a package-level index and is not a substitute. No implementation or production authorization follows from this draft.
